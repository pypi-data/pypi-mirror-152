...
from ai4water.backend import np, pd, plt, mpl


GridSpec = mpl.gridspec.GridSpec
GridSpecFromSubplotSpec = mpl.gridspec.GridSpecFromSubplotSpec


def prediction_distribution_plot(
        self,
        inputs, prediction, feature, feature_name, num_grid_points=10, grid_type='percentile',
    ...
    if self.mode == "regression":
        ...
        plot_classes = range(self.n_classes(prediction))
        if classes is not None:
            _check_classes(classes_list=classes, n_classes=self.n_classes(prediction))
