...
def update_audience_targeting(headers,list_of_ids,relation_in_set,relation_between_sets,exclude,placement_id):
...
def get_io_data(headers,io_id):
    import requests as rs
    from dict2xml import dict2xml
    import xmltodict
    from dict2xml import dict2xml

    url = f'https://api.freewheel.tv/services/v3/insertion_order/{io_id}'
    
    get_io = rs.get(url,headers=headers).text
    

    return xmltodict.parse(get_io,dict_constructor=dict)