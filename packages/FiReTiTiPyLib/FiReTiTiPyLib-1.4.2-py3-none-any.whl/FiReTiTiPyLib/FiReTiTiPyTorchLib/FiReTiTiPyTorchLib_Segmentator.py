...
from FiReTiTiPyLib import ImageTools as imTools
...
class _SingleIterator(object):
	...
	def __init__(self, Image, CropSize: int, BorderEffectSize: int, Iterator: bool=False, verbose=False):
		...
		self.Image = Image
		self.Width, self.Height, self.Channels, self.First = imTools.Dimensions(Image)
...
class Segment(object):
	...
	def PyTorch(self, ImagesDirPath: str, Model, Device, CropSize: int, BorderEffectSize: int,
		...
		imnames = imIO.FindImages(ImagesDirPath, NamesOnly=True, verbose=False)
		images = imIO.LoadImages(ImagesDirPath, ChannelFirst=True, ReturnImagesList=False, verbose=False)
		nbImages = len(images)
		...
		for i in range(nbImages):
			...
			if nbOuputs == 1:
				imIO.Write(result[0], True, ResultsDirPath + "/" + imnames[i].replace('.tiff','.png').replace('.tif','.png'))
			else:
				for o in range(nbOuputs):
					imIO.Write(result[o], True, ResultsDirPath + "/Out " + str(o)+"/"+imnames[i].replace('.tiff','.png').replace('.tif','.png'))
	...
	def MaskRCNN(self, ImagesDirPath: str, ImageNameFilter: str, Model, Device, CropSize: int, BorderEffectSize: int,
		...
		imnames = imIO.FindImages(ImagesDirPath, NamesOnly=True, verbose=False)
		impaths = imIO.FindImages(ImagesDirPath, NamesOnly=False, verbose=False)
		...
		for i in range(nbImages):
			...
			image = imIO.LoadImagesList([impaths[i]], ChannelFirst=True, ReturnImagesList=False, verbose=self.verbose)[0]
			width, height, channels, first = imTools.Dimensions(image)
			...
			for box in FinalBoxes:
				...
				if SaveIndividualObject == True:
					imIO.Write(crop, True, objdir + "/" + str(num) + "_" + str(bb[0]) + "x" + str(bb[1]) + ".png")
			...
			ext = " - Labels.png"
			imIO.Write(resLabels, True, ResultsDirPath + "/" + imnames[i].replace('.tiff', ext).replace('.tif', ext).replace('.png', ext))
			w, h = resBoxes.size
			ext = " - Boxes.png"
			...
			ext = " - Foreground.png"
			imIO.Write(resForeground.astype(numpy.uint8), True, ResultsDirPath + "/" + imnames[i].replace('.tiff', ext).replace('.tif', ext).replace('.png', ext))
