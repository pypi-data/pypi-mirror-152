...
def 帮助():
    ...
    print(logs.颜色(
        ...
        色选="红色"))
    Fishsys.系统日志(f"读取缓存信息Foreceupdata [{Forcedupdate}]", "强制更新系统help", UPI)
    Fishsys.系统日志(f"读取缓存信息sys[{sys}]", "强制更新系统help", UPI)

    Fishsys.系统日志(f"这是强制更新系统开启时的操作", "强制更新系统help", UPI)
    isConnected = Fishsys.官网存活性检测("https://pypi.org/project/Fishconsole/")
    if Forcedupdate:
        if isConnected:
            Fishsys.系统日志(f"检测到网络连接成功", "强制更新系统help", UPI)
            ...
            Fishsys.系统日志(f"res_v is {res_v}", "强制更新系统help", UPI)
            Fishsys.系统日志(f"v is {v}", "强制更新系统help", UPI)
            # 开启了强制更新系统
            if res_v == v:
                ...
                config = files.缓存(2)
                if config:
                    Fishsys.系统日志("这是缓存获取成功时执行的操作", "强制更新系统helps", UPI)
                    config["updata"] = False
                    files.缓存(1, config)
                    config["year"] = 2002
                    config["month"] = 6
                    ...
                    Fishsys.系统日志("这是缓存获取失败时执行的操作", "强制更新系统helps", UPI)
                    config = {"year": 2002, "month": 6, "day": 3, "updata": False}
                    files.缓存(1, config)
            else:
                # 对比到版本号不同
                Fishsys.系统日志(f"对比到版本号不同", "强制更新系统help", UPI)
                ...
                config = files.缓存(2)
                Fishsys.系统日志(f"config is {config}", "强制更新系统help", UPI)
                config["updata"] = True
                files.缓存(1, config)
                    ...
                    for a in res:
                        ...
                        if a == "缓存模块":
                            window.弹窗(
                                ...
                                "\n-----------------------------\n\n"
                                "语法就是\n\n"
                                "files.缓存(模式, '字典（请注意，字符串用单引号括起来，千万不要用双引号，否则将过不了加密',缓存文件的名字)\n\n"
