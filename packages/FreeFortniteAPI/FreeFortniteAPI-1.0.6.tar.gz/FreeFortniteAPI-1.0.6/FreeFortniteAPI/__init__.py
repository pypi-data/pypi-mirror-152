...
class FortniteAPI:
    ...
    #cosmetics by id
    def cosmetics_by_id(idc=None, type=None):
        if type == "id":
            try:
                return requests.get(f"https://fortnite-api.com/v2/cosmetics/br/{idc}").json()["data"]["id"]
            except:
                print("ERROR : can't connect to API please try again later.")

        elif type == "name":
            try:
                return requests.get(f"https://fortnite-api.com/v2/cosmetics/br/{idc}").json()["data"]["name"]
            except:
                print("ERROR : can't connect to API please try again later.")

        elif type == "description":
            try:
                return requests.get(f"https://fortnite-api.com/v2/cosmetics/br/{idc}").json()["data"]["description"]
            except:
                print("ERROR : can't connect to API please try again later.")

        elif type == "type":
            try:
                return requests.get(f"https://fortnite-api.com/v2/cosmetics/br/{idc}").json()["data"]["type"]["displayValue"]
            except:
                print("ERROR : can't connect to API please try again later.")

        elif type == "rarity":
            try:
                return requests.get(f"https://fortnite-api.com/v2/cosmetics/br/{idc}").json()["data"]["rarity"]["displayValue"]
            except:
                print("ERROR : can't connect to API please try again later.")

        elif type == "image":
            try:
                return requests.get(f"https://fortnite-api.com/v2/cosmetics/br/{idc}").json()["data"]["images"]["icon"]
            except:
                print("ERROR : can't connect to API please try again later.")

        elif type == "introduction":
            try:
                return requests.get(f"https://fortnite-api.com/v2/cosmetics/br/{idc}").json()["data"]["introduction"]["text"]
            except:
                print("ERROR : can't connect to API please try again later.")

        elif type == "file":
            try:
                return requests.get(f"https://fortnite-api.com/v2/cosmetics/br/{idc}").json()["data"]["path"]
            except:
                print("ERROR : can't connect to API please try again later.")
