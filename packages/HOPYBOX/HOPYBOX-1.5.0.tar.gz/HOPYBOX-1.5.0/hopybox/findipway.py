...
def way_four(ip):
  ...
  if '\', \'' in ipfway[1]:
    ...
    Error_pta('NotFoundIPError','Command','An IP address query error occurred','fwayget '+ip)
  elif ipfway[1] == '局域网对方和您在同一内部网':
    Error_pta('IPAddressError','Command','LAN\'s other parties and you are in the same internal network','fwayget '+ip)
  else:
