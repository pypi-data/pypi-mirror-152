...
class PolytopeReducer:
    ...
    def sparse_null_space(S_df):
        ...
        # in_matrix = SparseMatrix(S)
        # print(time.time()-start)
        # print("check")
        rref_m, rref_pivot = in_matrix.rref()
        # print("check")
        # show_structure_of_matrix(rref_m, 'rref.png')
        rank = len(rref_pivot)
        # pivoted = Matrix.zeros(rref_m.rows, rref_m.cols)
        rref_inverted = list()
