...
class StoichiometryParser:
    ...
    @staticmethod
    def make_precision_truncated_polytope(polytope, precision):
        ...
        for attribute in dir(polytope):
            ...
            if isinstance(tentative_df, pd.DataFrame) or isinstance(
                ...
                temp_df = ((tentative_df*precision).astype(int).astype(float))/precision
                setattr(polytope, attribute, temp_df)
        return polytope
