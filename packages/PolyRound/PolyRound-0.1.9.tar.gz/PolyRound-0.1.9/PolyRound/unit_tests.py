...
class TestCoordinateTransform(unittest.TestCase):
    ...
    def test_norm(self):
        ...
        for digits in [1, 10,10000]:
            reduced_p = StoichiometryParser.make_precision_truncated_polytope(polytope, digits)
            trans = PolytopeReducer.sparse_null_space(reduced_p.S)
            self.assertLessEqual(np.linalg.norm(reduced_p.S @ trans), 1e-10)
