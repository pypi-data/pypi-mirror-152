class Missing_value:
      def __init__(self,dataset,label):
          """
          When no information is supplied for one or more elements, or for
          the entire unit, this is referred to as missing data. In pandas,
          missing data may also be referred to as NA (Not Available) values.
          It is a critical issue in data analysis since it affects the results. 
          It may impair statistical power and lead to erroneous conclusions 
          due to biased values.
          """
      ...
      def get_plot(self,figsize=(20,5),grid=True,threshold=0): 
            ...
            Returns     
            ----------
            plot      : Returns with clean and interative visulization of missing value 
...
class Multicollinearity_Filter:
      ...
      def internal_operation(self): 
            ...
            ----------
            X                      : pandas.DataFrame
                                    Group of predictor feature that has some correlation
                                    between target features.It only contain list of 
                                    predictor feature to find out collinearity between them.

            allowable_collinearity : int/float
                                    It helps to keep multicollinearity between two 
                                    feature below the threshold values.Multicollinarity
                                    range can be effected by scaling range or method.
                                    So pass only scaled data.

            Returns
            -------
            output                 : pandas.DataFrame 
                                    It represents individual features from given table
                                    with there Multicolinearity and rank.It shows low
                                    collinarity Feature table and automatically remove
                                    remaining features.                                   
      ...
      def quick_plot(self,figsize=(20,5),grid=True):
            """
            Returns with Multicollinearity value plot (Bar plot). Its gives quick 
            review to user to take right decision base on graphical visualizations.

            Parameters
            ----------
            figsize   : int
                         Configure size of the plot by passing positive integers which
                         value varies between 0 to 50.It represents wide and height of
                         the plot. Default value of figsize is 20,5.
            ...
            Returns     
            ----------
            plot      : Returns with clean and interative visulization of 
                        Multicollinearity value report using bar chart.
            """ 
            import matplotlib.pyplot as plt
            import pandas as pd
            import numpy as np
...
class Variance_filter:
      ...
      def get_variance_report(self,dataset):
          ...
          details      : list or tuple
                        ...
                        Creat list of columns name before assigning to the variable 
                        name `lable`.

          dataset      : pandas.DataFrame     
                          Use only DataFrame which carry both predictor and target 
                          feature.Try to keep target feature as last columns.
          Returns
          -------
          output       : pandas.DataFrame 
                        It represents individual features from given table with there 
                        variance and rank.                                                       
          """
          import pandas as pd
          import numpy as np
      ...
      def get_filter_data(self,dataset,threshold):
          ...
          threshold    : int or float
                         Variance threshold value is used to divide dataframe into filter
                         and unfilter datasets.This Threshold value is varies from 0 to 100%.
                         Default value is 0 `zero`.
          Returns
          -------
          output       : pandas.DataFrame,pandas.DataFrame  
                         It represents individual features from given table with there 
                         variance and rank.It gives two dataframe/table which contains 
                         filter and unfilter dataset measuring parameters.
      ...
      def quick_plot(self,figsize=(20,5),grid=True):
            ...
            Returns     
            ----------
            plot      : Returns with clean and interative visulization of variance value 
                        report using bar chart.
            """ 
            import matplotlib.pyplot as plt
            import pandas as pd
...
class stats_correlation:
       def __init__(self):
           """
           The concept of correlation is used to describe the relationship between two 
           or more variables. A correlation coefficient is a useful tool for determining
           the nature and strength of the connection that exists between two variables. 
           Pearson's Correlation Coefficient (PCC) is the most often used correlation
           coefficient. Along with PCC, Spearman's Correlation Coefficient and Kendall
           are used for finding correlation between features.

           P-value: The P-value determines how strongly your data rejects the null 
           hypothesis, which claims that no link exists between the two groups 
           being compared. The correlation coefficient is statistically 
       ...
       def get_correlation(self,X,y,method='pearson',threshold=0.05):
           ...
           ----------
           X              : pandas.dataframe
                             It is the group of predictor feature has n_sample, n_feature 
                             configration.Individual column of the table get compaired with
                             target feature to find out correlation between them.

           y              : pandas.dataframe
                              It is target feature that compaired with all individual 
                              columns to findout best correlation. The correlation 
                              coefficent will be +ve or -ve correlation coefficents.

           method         : string or str
                              Its help to select different type of correlation method. 
                              It may be 'pearson','spearman','Kendall' correlation 
                              coefficients. method={pearson','spearman','Kendall'}  

           threshold      : int or float
                            This threshold value is based on P-values.The P-value 
                            determines how strongly your data rejects the null hypothesis, 
                            which claims that no link exists between the two groups being
                            compared. The correlation coefficient is statistically 
                            significant when the probability is less than the standard 
                            5% (P0.05). 
           Return
           ----------
           output         : pandas.dataframe
                            It gives separate table/dataframe which carry positive,
                            negative and absolute correlation coefficient and have the
                            measuring parameters like features,p-values,rank & 
       ...
       def quick_plot(self,figsize=(20,5),grid=True,method='positive'):
            """
            Returns with correlation value plot (Bar plot). Its gives quick review to user
            to take right decision base on graphical visualizations.
            ...
            method    : str or string
                        The `positive`, `negative` and `absolute` is type of the 
                        correlation which user can be plot quickly by defining 
                        string name to variable `method`.             

            Returns     
            ----------
            plot      :  Returns with clean and interative visulization of correlation 
                         value report using bar chart.
            """
            import matplotlib.pyplot as plt
            import pandas as pd
            import numpy as np
...
class F_Regression:
      def __init__(self):
         """
         The Fisher score is one of the supervised feature selection approaches that is 
         utilised the most frequently. The algorithm that we will employ will yield the
         rankings of the variables in a decreasing order depending on the fisher's score.
         """
         pass

      def get_F_score(self,X,y,threshold=0.05):
          """
           Return with fisher score of the individual features and rank them based on 
           p-value.
           ...
           X              : pandas.dataframe
                             It is the group of predictor feature has n_sample, n_feature
                             configration.Individual column of the table get compaired 
                             with target feature to find out fisher score between them.

           y              : pandas.dataframe
                              It is target feature that compaired with all individual 
                              columns to findout best correlation. It gives fisher 
                              score and corresponding p-value that represents individual 
                              feature importance. 
           ...
           ----------
           output         : pandas.dataframe
                             It gives separate table which carry fisher score and p-value 
                             of individual feature and have the measuring parameters like 
                             features,p-values,rank & fisher scores.
          """
          import pandas as pd
          import numpy as np
      ...
      def quick_plot(self,figsize=(20,5),grid=True): 
            ...
            Returns     
            ----------
            plot      : Returns with clean and interative visulization of fisher score 
                        report using bar chart.
            """
            import matplotlib.pyplot as plt
            import pandas as pd
...
class Analysis_variance_test:
      def __init__(self):
            """
            The analysis of variance, often known as ANOVA, is a statistical method that
            compares the means of two or more groups to determine whether or not there 
            is a significant gap between them. The analysis of variance (ANOVA) examines
            the effect of one or more factors by contrasting the means of several samples.
            """
            pass
      def get_report(self,dataset,columns,threshold=0.05):
          """
          Return with stat-value, p-values, status and feature ranking using ANOVA 
          (Analysis of variance) Test.

          Parameters
          ----------
          dataset        : pandas.dataframe
                             It is complete dataframe which carry predictor and target 
                             features. Keep in the mind that last column present in the
                             dataset should be target feature.It has n_sample,n_feature 
                             configrations.

          columns        : list or tuple
                            The list or group of the columns names here assigned to 
                            variable name `columns`.It helps to assigne right lable to
                            right feature. 
          ...
          ----------
          output         : pandas.dataframe
                             It gives separate table which carry stat-value, p-value of 
                             individual feature and have the measuring parameters like 
                             features,p-values,rank & stat-value.
      ...
      def apply_filter(self,threshold=0.05):
          ...
          threshold      : int or float
                            ...
                            determines how strongly your data rejects the null hypothesis,
                            which claims that no link exists between the two groups being
                            compared. The correlation coefficient is statistically significant
          ...
          output         : pandas.dataframe,pandas.dataframe
                             It gives separate table which carry stat-value, p-value of
                             individual feature and have the measuring parameters like 
                             features,p-values,rank & stat-value. It gives us filter &
                             unfilter dataframes which describes distributions of 
                             individual feature.
          """
          import numpy as np
          import pandas as pd
      ...
      def get_plot(self,X_label='feature',y_label='weight(%)',figsize=(20,5),grid=False,threshold=1.0,labels=False,legend=False,dpi=100): 
          """ 
            Returns with fisher score plot (Bar plot). Its gives quick review to user 
            to take right decision base on graphical visualizations.

            Parameters
            ----------
            X_label   : str
                        The label given for x-axis.It assigne `feature` name along
                         X-axis.

            y_label   : str
                        It plot for weight(%) and p-value VS feature plot after 
                        selecting label using variable `y_label`.
            ...
            labels    : boolean
                         The value assigned to the label variable `True` or `false` is 
                         descided wether labels are included in plot or not.

            legend    : boolean
                         The boolean value of the legend descided wether variable legend
                         are included in plot or not.
...
class Forward_Feature_Selection:
       """
       Forward selection is an iterative process that begins with the model containing 
       no features at the initial stage (at zero iteration). This method keeps adding 
       the features that best improve model performance in each successive iteration 
       until adding a new variable no further enhances the model's performance. This 
       feature selection method helps to reduce model training time, provide good 
       fitting by reducing overfitting issue and minimises the impact of the
       ...
       def get_result(self):
            ...
            Parameters
            ----------
            model       : Define the machine learning model to perform forward feature 
                          selection. It select most significant features based on 
                          r2_score and mean square error value.  

            X_train     : {array-like, sparse matrix} of shape (n_samples, n_features)
                             The input samples. It is subset of main dataset which 
                             represent set of independent Features.
            ...
            interation  : int
                             It decside number of iteration perform by forward 
                             feature selection method to get most relevent 
                             features from it.

            X_label     : list or tuple
                             The group/list of columns name can be assigned Here as
                             `X_label`. Creat list of columns name before assigning 
                             to the variable name `X_label`.

            Return
            --------
            output      : pandas.dataframe
                             It gives output in form of table/dataframe which carry 
                             parameters like feature,mean square error and R2_score value.                 
            """
            import pandas as pd
            import numpy as np
            import random
       ...
       def get_plot(self,figsize=(20,8)):
            """
            Returns with most significant feature and represent it with line plot. 
            Its gives quick review to user to take right decision base on graphical
            visualizations.
            ...
            Returns     
            ----------
            plot      : Returns with clean and interative visulization of most 
                         significant feature with there mean square error and r2_score
                         value using line chart.
            """
            import pandas as pd
            import numpy as np
            import matplotlib.pyplot as plt
...
class backword_feature_elimination:
      """
      Backward feature elimination is used to eliminate characteristics that have no 
      influence on the dependent variable or prediction of output. Feature selection 
      begin by constructing a model with all of the characteristics that are 
      currently available. In subsequent stages the features those who are having 
      least or no impact on maodel performance is eliminated.
      """
      ...
      def get_report(self):
          """
            ...
            Parameters
            ----------
            model       : Define the machine learning model to perform backword feature
                          elimination. It select most significant features based on 
                          r2_score and mean square error value.  

            X_train     : {array-like, sparse matrix} of shape (n_samples, n_features)
                          The input samples. It is subset of main dataset which 
                          represent set of independent Features.It use for to train
                          the model.

            X_test      :{array-like, sparse matrix} of shape (n_samples, n_features)
                            The input samples. It is subset of main dataset which 
                            represent set of independent Features.It use for to test
                            the model.

            y_train     :{array-like, sparse matrix} of shape (n_samples)
                            The input samples. It is subset of main dataset which
                            represent target Features.It use for to train the model.

            y_test      :{array-like, sparse matrix} of shape (n_samples)
                            The input samples. It is subset of main dataset which
                            represent target Features.It use for to test the model.
            ...
            y_label     : list or tuple
                          The group/list of columns name can be assigned Here as
                          `y_label`. Creat list of columns name before assigning 
                          to the variable name `y_label`.

            Return
            --------
            output      : pandas.dataframe
                             It gives output in form of table/dataframe which carry 
                             parameters like feature,mean square error and R2_score 
                             value.          
          """ 
          import pandas as pd
      ...
      def get_default_plot(self,grid=True,figsize=(18,6)):
            ...
            ----------
            figsize   : int
                         Configure size of the plot by passing positive integers 
                         which value varies between 0 to 50.It represents wide and
                         height of the plot. Default value of figsize is 20,5.
            ...
            ----------
            plot      :  Returns with clean and interative visulization of most 
                         significant feature with there mean square error and
                         r2_score value using line chart.
            """
            import matplotlib.pyplot as plt
            import pandas as pd
            import numpy as np
...
class HeatMap:
      def __init__(self,dataset,figsize=(20,15),method='pearson',templet=2):
          """
            Returns with correlation value plot (Bar plot). Its gives quick review 
            to user to take right decision base on graphical visualizations.
            ...
            templet   : int
                        It has inbuild 177 templet. To access different templets user
                        can pass positive integer to varible name as `templet`. 
                        Its range varies from 0 to 177.                               
      ...
      def correlation_matrics(self):
          """
            Returns with most significant feature and represent it with heat-map. 
            Its gives quick review to user to take right decision base on 
...
class chi_square:
      def __init__(self,dataset,columns):
          """
          Return with chi_score, p-values, status and feature ranking using 
          chi_square Test.

          Parameters
          ----------
          dataset        : pandas.dataframe
                             It is complete dataframe which carry predictor and 
                             target features.Keep in the mind that last column 
                             present in the dataset should be target feature.
                             It has n_sample,n_feature configrations.

          columns        : list or tuple
                              The list or group of the columns names here assigned
                              to variable name `columns`.It helps to assigne right
                              lable to right feature. 

          threshold      : int or float
                             This threshold value is based on P-values.The 
                             P-value determines how strongly your data 
                             rejects the null hypothesis, which claims that
                             no link exists between the two groups being compared.
                             The correlation coefficient is statistically 
                             significant when the probabilityis less than the standard
                             5% (P0.05). 
          Return
          ----------
          output         : pandas.dataframe
                            It gives separate table which carry chi_score, p-value
                            of individual feature and have the measuring parameters
                            like features,p-values,rank & chi_score.
          """
          import numpy as np
          import pandas as pd
      ...
      def get_report(self):
            ...
            threshold      : int or float
                            This threshold value is based on P-values.The P-value
                            determines how strongly your data rejects the null 
                            hypothesis, which claims that no link exists between 
                            the two groups being compared. The correlation 
                            coefficient is statistically significant when the 
                            probability is less than the standard 5% (P0.05). 
            Return
            ----------
            output         : pandas.dataframe
                            It gives separate table which carry chi_score, p-value 
      ...
      def apply_filter(self,threshold=0.5):
          """
          Return with stat-value, p-values, status and feature ranking using 
          ANOVA (Analysis of variance) Test.

          Parameters
          ----------
          threshold      : int or float
                            This threshold value is based on `p-values`. 
                            The `P-value` determines how strongly your data rejects
                            the null hypothesis, which claims that no link exists 
                            between the two groups being compared. The correlation
                            coefficient is statistically significant when the 
                            probability is less than the standard 5% (P0.05). 
          Return
          ----------
          output         : pandas.dataframe,pandas.dataframe
                            It gives separate table which carry stat-value, 
                            p-value of individual feature and have the
                            measuring parameters like features,p-values,rank & 
                            stat-value. It gives us filter & unfilter dataframes
                            which describes distributions of individual feature.          
          """
          import numpy as np
          import pandas as pd
      ...
      def get_plot(self,X_label='Feature',y_label='chi_score',figsize=(20,5),grid=False,threshold=1.0,labels=False,legend=False,dpi=100): 
          ...
          ----------
          X_label        : str
                           The label given for x-axis.It assigne `feature`
                           name along X-axis.
          ...
          labels         : boolean
                             The value assigned to the label variable `True` or `false`
                             is descided wether labels are included in plot or not.
