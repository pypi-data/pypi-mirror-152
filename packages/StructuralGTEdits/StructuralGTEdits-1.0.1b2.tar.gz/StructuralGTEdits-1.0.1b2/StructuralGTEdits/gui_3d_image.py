...
def getfile():
    ...
    if afile.endswith(('.tif', '.tiff', '.png', '.jpg', '.jpeg')):
    ...
    # file has to be an image that we can work with
        ...
        direntry.delete('1.0', END)
        direntry.insert('end', 'File needs to be a .tif, .png, or .jpg')
        direntry.config(font=("Helvetica", 14, "bold"))
        direntry.config(state='disabled')


def getstack():
    
    # Get directory which contains stack of .tiff files
    adir = filedialog.askdirectory()

    # Replace with test condition 
    if True:
        ...
        # splitting the directory location into its name and path and displaying
        path, filename = os.path.split(adir)
        fileentry.config(state='normal')
        fileentry.delete('1.0', END)
        fileentry.insert('end', adir)
        fileentry.config(font=("Helvetica", 12, "italic"))
        fileentry.config(state='disabled')
    ...
    # dir has to be an image that we can work with
    if False:
        fileentry.config(state='normal')
        direntry.delete('1.0', END)
        direntry.insert('end', 'File needs to be a .tif, .png, or .jpg')
...
def Confirm_button_ex():
    ...
    settings_3d.make_settings(root, src, saveloc, filename)


def Confirm_nocrop_button():

    # Change for 3D: src now a stack of imgs
    ...
    _dir = os.path.join(path, filename)
    print(path)
    print(filename)
    print(_dir)

    stack=[]
    for i in np.arange(len(os.listdir('1'))):
        stack.append(cv2.imread(_dir+'/3d'+str(i)+'.tiff',cv2.IMREAD_GRAYSCALE))
   
    w, h =(stack[0].shape[0]),(stack[0].shape[1])
    print(stack)
    # TODO: Currently assumes all images the same size and thus require the same scaling
    if (h >w):
        scalefactor = 512 / h
    else:
        scalefactor = 512 / w
    ...
    for i in np.arange(len(stack)-1):
        src.append(cv2.resize(stack[i], dsize))
...
def newimwindow():
    ...
    button6 = Button(frame4, text="Update", command=lambda: draw_rect())
...
def make_gui(window):
    ...
    button0 = Button(frame3, text="Proceed with crop", bg="Green", command= lambda: Confirm_button_exit())
    ...
    button3 = Button(frame2, text="Select stack..", bg="gray", command= lambda: getstack())
    button4 = Button(frame2, text="Choose Save Location...", bg="gray", command= lambda: choosesave())
    button5 = Button(frame2, text="Crop Image", bg="gray", command= lambda: newimwindow())
