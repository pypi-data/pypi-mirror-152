...
def save_data_3d(src, Thresh_method, gamma, md_filter, g_blur, autolvl, fg_color, asize, bsize, wsize, thresh, laplacian, \
    ...
    #3D Currently, only change is that src is stack of tiffs, not a single image

    # Updating the label alert
    update_label("Processing image...")

    #3D output is (img, img_bin, ret) for single image, within the image stack, stack. I have tried to accomplish this assignment with generators but till now cannot.
    stack=src
    img=[]
    img_bin=[]
    ret=[]
    for i in np.arange(len(stack)):
        output = process_image.binarize(stack[i])
        img.append(output[0])
        img_bin.append(output[1])
        ret.append(output[2])
    
    progress(10)
    update_label("Extracting graph...")

    #3D The modified skel_ID function returns the complete set of results for a stack, after only being called once, as opposed to binarize() which must be called several times
    # newskel is just cleaned skel (if applicabl)
    newskel, skel_int, Bp_coord_x, Bp_coord_y, Bp_coord_z, Ep_coord_x, Ep_coord_y, Ep_coord_z = \
    skel_ID_3d.make_skel(img_bin, merge_nodes, prune, clean, r_size)
    ...
    img_filt = img
    img_bin = img_bin
    histo = cv2.calcHist([img_filt[0]], [0], None, [256], [0, 256]) #3D Just takes first image
...
def Proceed_button():
    ...
    save_data_3d(src, Thresh_method, gamma, md_filter, g_blur, autolvl, fg_color, asize, bsize, wsize, thresh, \
...
def make_settings(root, source, saveloc, filename):
    ...
    # making the new filenames -- filename is actually a dirname for 3d analysis. So remove....
#    filename = re.sub('.png', '', filename)
#    filename = re.sub('.tif', '', filename)
#    filename = re.sub('.jpg', '', filename)
#    filename = re.sub('.jpeg', '', filename)
...
#    file2 = os.path.join(saveloc, ELfile)
    ...
    img1 = Image.fromarray(img_bin[0]) #3D Just takes first image from img_bin
