...
class RPC:
    ...
    def call(self, method, **kwargs):
        ...
        if state:
                ...
                if "headers" in kwargs:
                    if isinstance(kwargs["headers"], dict):
                        for o in kwargs["headers"]:
                            m_headers[
                                o.lower().replace("http_", "").replace("_", "-")
                            ] = kwargs["headers"][o]
                    else:
                        return {
                            "state": -1,
                    ...
                    if param["key"].lower() != "pk":
                        if param["key"].startswith("http_header_") or param[
                            ...
                            if 'data' not in kwargs:
                                kwargs["data"]={}
                            if param['key'].replace('http_data_',"") not in kwargs['data']:
                                kwargs[param['key'].replace('http_data_',"")]=None
                            kwargs["data"][param['key'].replace('http_data_',"")]=kwargs[param['key']]
                        else:
                            if param["key"] in kwargs:
                                params_data += (
                                    ...
                                    + "="
                                    + (
                                        parse.quote(kwargs[param["key"]])
                                        if isinstance(kwargs[param["key"]], str)
                                        else parse.quote(str(kwargs[param["key"]]))
                                    )
                                )
                            else:
                                    ...
                                    if param["defaultValue"]:
                                        params_data += (
                                            ...
                                            + (
                                                ...
                                                if isinstance(param["defaultValue"], str)
                                                else parse.quote(str(param["defaultValue"]))
                                            )
                                        )
                                else:
                                    return {"state": -1, "msg": "not found '%s' paramete."%(param["key"])}
                            m_count += 1
            ...
            try:
                if m_method == u"GET":
                    logging.info("access api:(%s), method(%s)." % (m_uri, m_method))
                    ...
                    if state == 200:
                        try:
                            ...
                            return {
                                "state": state,
                                "msg": 'remote call "%s" error.(%s)' % (m_uri,e.args),
                            }
                    else:
                        try:
                            if ret:
                    ...
                    logging.info(
                        "access api:(%s), method(%s),postdata:%s."
                    ...
                    return {"state":-1, "msg":"restful method(%s) is error. POST|GET|DELETE|PUT(unrealized)|OPTIONS(unrealized)|HEAD(unrealized)"%(m_method)}
            except Exception as e:
                return {"state": -1, "msg": e.args}
        else:
