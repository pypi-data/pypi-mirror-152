        ...
        if self.imagePath != None:
            ...
            if self.imagePath.endswith((".png", ".tiff", "tif")):
                ...
                if ok and FOV:
                    ...
                    self.minMaxNeuronValues = [i * (zoom_factor**2) for i in [25,700]]

                    minArea, maxArea = self.minMaxNeuronValues

                    pred_main(im_path, model=self.currentModel, minimal_area_neuron_in_pixels=minArea,
                              maximal_area_neuron_in_pixels=maxArea, zoom_factor=1/zoom_factor)
    ...
    def extractTraces(self, imagesArray):  # , frequency):
        ...
        if ok and fps:
            ...
            lfl, ok = QtWidgets.QInputDialog().getInt(self,
                                                         "Dynamic Baseline Estimation (rolling window length)",
                                                         "Please provide the wished rolling window size [seconds] for the baseline calculation (default: 6s)",
                                                         6, 0, 500)

            if ok and lfl:

                dff_calc(self.filename.replace(".json", "_traces.tsv"), fps, lfl)
            ...
            mbFormat = QtWidgets.QMessageBox(QtWidgets.QMessageBox.Warning, "No FPS were given",
