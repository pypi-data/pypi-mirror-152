...
class InstanceManager(ManagedAuthentication):
    ...
    def provision(
        self, name, memory: float = 1, cpu: float = 1, ports=None,
            command=['/bin/sh' ,'-c', f'apt-get update && apt-get install python3 python3-pip python3-dev -y && python3 -m pip install acido=={__version__} && sleep infinity'], image = 'ubuntu:20.04',
            env_vars: dict = {}
    ):
        env = {}
