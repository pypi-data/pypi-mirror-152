...
def exec_command(rg, cg, cont, command, max_retries, input_file):
    ...
    # Kill tmux window
    subprocess.Popen(["tmux", "kill-session", "-t", cont], env=env, stdout=subprocess.DEVNULL)
    time.sleep(2)
...
class Acido(object):
    ...
    def load_input(self, command_uuid: str = None, filename: str = 'input.txt', write_to_file: bool = False):
        if command_uuid:
        ...
        if input_file:
            ...
            for file in os.listdir(directory):
                ...
                if filename.startswith("acido-input"): 
                    input_files.append(self.save_input(filename))
                else:
                    continue
