...
#         hook.run(sql, params)
#         break
def update_rows(hook, table, df, primary_keys):
    update_sql = f"UPDATE {table} SET "
    column_names = df.columns.tolist()
    set_clause =  ', '.join([f'{key} = $' + str(index + 1) for index, key in enumerate(column_names) if key not in primary_keys])
    where_clause = ' AND '.join([f'{key} = $' + str(i+len(column_names)) for i, key in enumerate(primary_keys, 1)])
    for index, row in df.iterrows():
        params = tuple([row[key] for key in column_names].append(row[key] for key in primary_keys))
        break
    sql = f"{update_sql} {set_clause} WHERE {where_clause};"
    print(f'SQL: {sql} \nParams: {params}\n')
