...
class BaseProject:
    ...
    Methods will perform 1)plotting data, 2)generating ``Experiment``s,
    3)setting models across the generated `Experiment`s, 4)setting trainers across the generated ``Experiment``s,
    5)training models across the generated `Experiment`s, 6)updating project metadata,
    7)adding individual ``Experiment`` to the project, 8)saving checkpoints for ``Experiment``s saved in ``exps``,
    ...
    Attributes:
        ...
        exps (:obj:`dict` of :obj:`str`, :obj:`Experiment`): `Project` can control ``Experiment``s in ``exps``.
    ...
    def create_experiments(
        ...
        r"""Generates ``Experiment``s and stack them to ``exps`` if needed.

        Args:
            
            amount (int) : The number of ``Experiment``s you want to generate.
                Default: ``10``
            
            test_size (float) : Test size when splitting the data. Neccessary for creating ``Experiment``s.
                Default: ``1e-2``
            
            stack_models (bool) : Whehter to stack models in each ``Experiment`` or not.
                Default: ``True``
            
            stack_exps (bool) : Whether to stack generated ``Experiment``s to ``exps`` or not.
                Default: ``True``
        ...
        for i in range(amount):
            ...
            exp_parameters.update({f'random_number' : seed})
    ...
    def add_experiment(
        ...
        if random_state in self.random_states:
            raise ValueError(f'There is already an "Experiment" with the same random state.')
    ...
    def set_models(
        ...
        **hparams
        ...
        r"""Set identical models across ``Experiment``s.
        ...
        Args:
            ...
            **hparams (Any): Hyperparameters for the model. 
            ...
            random_states (:obj:`list` of `int`, optional): The list of random states of ``Experiment``s you want to set model to. If None, will set models for all ``Experiment``s in ``exps``.
                Default: ``None``
    ...
    def set_trainers(
        ...
        **trainer_config
        ...
        r"""Set identical trainers across ``Experiment``s.

        Args:

            **trainer_config (Any): Trainer configurations.

            random_states (:obj:`list` of `int`, optional): The list of random states of ``Experiment``s you want to set trainer to. If None, will set trainers for all ``Experiment``s in ``exps``.
                Default: ``None``
    ...
    def fit_experiments(
        ...
        **trainer_config
        ...
        r"""Fit models in ``Experiment``s stacked in ``exps``.

        Args:

            **trainer_config (Any): Trainer configurations.

            random_states (:obj:`list` of `int`, optional): The list of random states of ``Experiment``s you want to fit. If None, will fit all ``Experiment``s in ``exps``.
                Default: ``None``
    ...
    def save_ckpts(
        ...
        r"""Save checkpoints in ``Experiment``s stacked in ``exps``.

        Args:

            random_states (:obj:`list` of `int`, optional): The list of random states of ``Experiment``s you want to save checkpoint. If None, will save checkpoint for all ``Experiment``s in ``exps``.
                Default: ``None``
    ...
    def save_stacks(
        ...
        r"""Apply `save_stack` for ``Experiment``s in ``exps``.

        Args:

            random_states (:obj:`list` of `int`, optional): The list of random states of ``Experiment``s you want to save stack. If None, will save stack for all ``Experiment``s in ``exps``.
                Default: ``None``
    ...
    def plot_data(
        ...
        if domain_type == "points": # data points
            if self.dims == 1: # 1D
                ...
                plt.xlabel(f'1st principal axis')
                plt.ylabel(f'2nd principal axis')
                plt.legend(loc=(1.,1.))
                plt.show()
                plt.savefig(points_dir+ f"/points_distribution_PCA.png")  
                self.project_logger.add_figure(f'data distribution_PCA', fig)
            ...
            img_dir = self.project_path + f"/visualizations"
            ...
            # for logging
            self.project_logger.add_images(f'sample_images', imgs_to_show, dataformats=dataformats)
            dataformats_without_N = dataformats.replace("N","")
            for i in range(n_data):
                self.project_logger.add_image(
                    f"sample_images/image_{i}_label_{labels_to_show[i]}",
                    imgs_to_show[i],
    ...
    def get_available_random_states(
        ...
        r"""Returns list of directories of ``Experiment``s that has been fitted and logged.

        Returns:

            list of `str`: All directories of fitted and logged ``Experiment``s.
        """
    ...
    def search_standard(
        ...
        Returns:
            ...
            tuple: (numpy ndarray of validation performances across ``Experiment``s, dictionary of {model type:it's frequency}, dictionary of {model structure:it's frequency}).
...
class Project(BaseProject):
    ...
    def plot_identical_model_type(
        ...
        Returns:
            ...
            tuple: (numpy ndarray of validation performances, list of random states of saved ``Experiment``s, dictionary containing information of the search)
        """
    ...
    def plot_identical_model_structure(
        ...
        **hparams
        ...
        Returns:
            ...
            tuple: (numpy ndarray of validation performances, list of random states of saved ``Experiment``s)
        """
    ...
    def generate_marker_style(amount)->List[str]: 
        # string specifying the marker style
    ...
    def plot_all_models(
        ...
        r"""Plots validation performances across different random states for all model structures saved. Will plot 1)line graphs and 2)box plots.
        ...
        Returns:
            ...
            tuple: (numpy ndarray of validation performances, list of random states of saved ``Experiment``s)
        """
    # (validation performances, list of model metadata)
