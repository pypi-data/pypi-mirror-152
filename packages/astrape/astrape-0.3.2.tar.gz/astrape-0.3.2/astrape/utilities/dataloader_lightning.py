...
class DataModule(pl.LightningDataModule):
    ...
    def setup(self, stage):
        if self.X_val is not None and self.y_val is not None and self.X_test is not None and self.y_test is not None:
            ...
            self.X_train_tensor, self.y_train_tensor, self.X_val_tensor, self.y_val_tensor, \
                self.X_predict_tensor, self.y_predict_tensor, self.X_test_tensor, self.y_test_tensor = \
            split_data(
                X=self.X, 
                y=self.y,
                X_test=self.X_test,
