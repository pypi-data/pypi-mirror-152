...
class BaseExperiment:
    ...
    def update_log_path(self)->None:
        r"""Updates log path"""
        self.exp_path = self.project_path + "/"+ self.fit_or_cv + "/" + f"random_state-{self.random_state}"
    ...
    def save_ckpt(
        ...
        ckpt_path = checkpoint_path +"/" + ckpt_name + f"version_{ckpt_version}"
        ...
        self._create_folder(metadata_dir+f"/version_{jsonlog_version}")
        if isinstance(model, BaseEstimator):
        ...
        self.n_ckpts += 1 
        self.log_metrics_to_json(model, tensorboard_logdir, jsonlog_version, val_metrics)
        self.update_exp_metadata()
    ...
    def best_ckpt_thus_far(
        ...
        try:
            ...
            logger.info("Nothing has been saved yet.")
            return None
        if not mode:
    ...
    def log_metrics_to_json(
        ...
        Args:
            ...
            version (int): The version of the model. >0 when identical models are trained and logged.
        ...
        jsonlog_folder = f"{self.log_path}/jsonlogs/{model.__class__.__name__}/{model_metadata}/version_{version}"
        self._create_folder(jsonlog_folder) # necessary for sklearn-based models
    ...
    def _dict_to_str(dictionary : dict)->str:
        r"""Polishes string form of the dictionary. Applied when defining hparams and using **hparams to pass arguments."""
        out = str(dictionary).replace("{", "").replace("}","").replace(" ","-").replace(":","").replace("-","").replace("\n", "-").replace('"', "").replace(".","d").replace("'","")
        return out

    def _args_to_str(*args)->str:
        r"""Polishes string form of the args when not. Applited when passing arguments as *args"""
        out = str(args).replace(",","_")
        return out