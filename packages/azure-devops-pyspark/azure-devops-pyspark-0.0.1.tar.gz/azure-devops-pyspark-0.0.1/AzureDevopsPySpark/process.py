...
class Process:
    @classmethod
    def columns(cls, data: dict, fields_column, fields_display) -> dict:
        """
            Receives a dictionary with the data of a work item and returns the data with the selected fields.
        """
        fields = [
          {column.name: data[tag]['displayName']} if tag in fields_display and tag in data \
              else ({column.name: data[tag]} if tag in data else {column.name: None}) for tag, column in fields_column.items()]
      
        date_type_fields = [schema.name for name, schema in fields_column.items() if schema.dataType == DateType()]
        schema_convert_date = [
            {
                name: cls.datetime_to_date(schema)
            } if name in date_type_fields and schema != None else {name:schema} for item in fields for name, schema in item.items()]
        item = {}
    ...
    def team_interation(cls, response: dict, squad: str) -> List[dict]:
        ...
        comprehension = [
            ...
            'Iteration_Start_Date': cls.datetime_to_date(values['attributes']['startDate']) if values['attributes']['startDate'] else None,
            'Iteration_End_Date': cls.datetime_to_date(values['attributes']['finishDate']) if values['attributes']['finishDate'] else None,
    ...
    @staticmethod
    def datetime_to_date(str_datetime: str):
        """
    ...
    def all_process(cls, response: dict) -> List[dict]:
        """
            ...
            [{'id':...,'name': Basic}]
        """
        return [{'id':values['typeId'], 'name':values['name']} for values in response['value']]
    ...
    def clean_fields(cls, response: dict) -> List[dict]:
        ...
        """
        return [{'referenceName': values['referenceName'], 'name': values['name'], 'type': values['type']} for values in response['value']]
