...
def load_dataset_for_labeling_service(
    ...
    with log_utils.log_activity(
        ...
        try:
            empty_file_cnt = 0  # count empty files
            final_conll_path = os.path.join(data_dir, data_filename)
            original_file_paths = []
            processed_input_file_counter = 1
            for i, ind in enumerate(text_df.index):
                at_start = i > empty_file_cnt
                # i is "how many files have been processed"
                # i > empty_file_cnt means at least one processed file is not empty
                portable_path = text_df[DataLabelingLiterals.PORTABLE_PATH_COLUMN_NAME][ind]
                ...
                if data_split == Split.test:
                    empty_file_cnt += _convert_to_conll_no_label(input_text_content, final_conll_path, at_start)
                else:
                    label_span = text_df[DataLiterals.LABEL_COLUMN][ind]
                    empty_file_cnt += _convert_to_conll(input_text_content, label_span, final_conll_path, at_start)
                processed_input_file_counter += 1
