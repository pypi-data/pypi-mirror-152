...
class LabelingDataHelperTest(unittest.TestCase):
    ...
    def test_load_dataset_for_labeling_service(self):
        ...
        with patch("builtins.open", new=open_mock):
            output_filename, input_file_paths = load_dataset_for_labeling_service(
                mock_dataset, data_dir, data_filename, data_label
            )
        ...
        # Check portable path
        self.assertEquals(
            input_file_paths, [portable_paths[0].lstrip("/"), portable_paths[1].lstrip("/")] * 25
        )

    def test_load_test_dataset_for_labeling_service(self):
        data_dir = DataLiterals.NER_DATA_DIR
        ...
        # Check portable path
        self.assertEquals(
            input_file_paths, [portable_paths[0].lstrip("/"), portable_paths[1][1:]] * 25
        )
