...
class BayesKMeans(object):
    ...
    def choose_optimization_method(self, optimized_method):
        """
            ...
            optimized_method : String
                Название метода оптимизации. Может принимать значения 'silhouette' и 'daviesBouldin'
        """
    ...
    def __get_elbow(self, n_clusters):
        """
            Внутренний метод класса. Возвращает значение метода локтя для разбиения набора данных
            на n_clusters кластеров.
            Parameters
            ----------
            n_clusters : int
            ...
            Returns
            -------
            double - значение метода Elbow для указанного количества кластеров
    ...
    def determine_right_border(self):
        """
            Определяет правую границу поиска байесовской оптимизации с помощью метода Локтя.
            Устанавливает значения внутренних переменных self.params.bound, self.params.x_0, self.params.y_0
        """
        step = 50
        diff = 20
        level = 0.75
        ...
        x0 = []
        y0 = []
        for i in range(len(self.params.x_0)):
            if self.params.x_0[i] <= border:
                x0.append([self.params.x_0[i]])
                y0.append(self.params.y_0[i])
        self.params.x_0 = x0
        self.params.y_0 = y0
    ...
    def __bayesian_optimization(self):
        """
            Запускает процесс определения количества кластеров байесовской оптимизацией.
            Устанавливает значение параметров self.x_iters, self.func_vals, self.found_k и self.min_function_value
        """
