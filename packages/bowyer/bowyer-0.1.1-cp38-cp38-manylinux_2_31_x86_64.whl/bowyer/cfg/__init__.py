...
import bowyer.cfg.data
import bowyer.cfg.edge
import bowyer.cfg.exception
import bowyer.cfg.load
import bowyer.cfg.override
import bowyer.cfg.queue
import bowyer.cfg.validate
import bowyer.util
import bowyer.util.serialization

from bowyer.cfg.exception import CfgError
...
def prepare(path_cfg       = None,  # pylint: disable=R0913
    ...
    if has_cfg_files:
        cfg_from_path = bowyer.cfg.load.from_path(path_cfg)
        if cfg_from_path:
            cfg = merge_dicts(cfg, cfg_from_path)

    if has_cfg_string:
        cfg_from_string = bowyer.util.serialization.deserialize(string_cfg)
        if cfg_from_string:
            cfg = merge_dicts(cfg, cfg_from_string)

    cfg = bowyer.cfg.override.apply(cfg            = cfg,
                                  tup_overrides  = tup_overrides,
                                  delim_cfg_addr = delim_cfg_addr)

    id_cfg = bowyer.util.serialization.hexdigest(cfg)[0:8]
    ...
    # cfg = bowyer.util.format_all_strings(cfg)
    cfg = bowyer.cfg.validate.normalized(cfg)

    if cfg is None:
        raise bowyer.cfg.exception.CfgError('No config.')
...
def denormalize(cfg):
    ...
    """
    return  bowyer.cfg.queue.denormalize(
                bowyer.cfg.edge.denormalize(
                    bowyer.cfg.data.denormalize(cfg)))
