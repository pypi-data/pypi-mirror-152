...
import bowyer.cfg.data.atomic_types
import bowyer.cfg.data.gap_table
import bowyer.cfg.util
import bowyer.util
...
def denormalize(cfg):
    ...
    cfg_data   = cfg['data']
    parameters = dict()
    subs       = bowyer.cfg.util.SubstitutionTable(parameters)
    typeinfo   = bowyer.cfg.data.atomic_types.as_dict()
    gap_table  = bowyer.cfg.data.gap_table.GapTable()
    stack      = _init_stack(map = cfg_data)
    expanded   = bowyer.util.PathDict()
...
def _init_stack(map):
    ...
    list_items = sorted(tup_items,
                        key = lambda item: bowyer.util.first(item.keys()))
...
def _iter_depth_first(stack):
    ...
    while True:  # Loop until stack empty. (Throws IndexError).
        ...
        field_name = bowyer.util.first(frame.item.keys())
        content    = bowyer.util.first(frame.item.values())
