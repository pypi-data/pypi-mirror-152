...
def xin2pbn(src_url, output):
    #print("wahtttttttt")
    #print(src_url)
    hacked_url=src_url.replace("#1111","")
    url = hacked_url.replace("&amp;","&").replace(";","%3B")
    o = urlparse(url)
    ...
    myuri = pathlib.Path(mypath).as_uri()
    # local can't contain #1111;
    localuri = hacked_url.replace("http://www.xinruibridge.com", myuri)
    template = pkgutil.get_data(__name__,'template.html')
    src = Template(template.decode('utf-8'))
    outfile = output + ".html"
    all["localuri"] = localuri
    all["originurl"] = src_url
    result = src.safe_substitute(all)
    with open(outfile, "w", encoding="utf-8") as text_file:
        print("write to file %s" % outfile)
        text_file.write(result)
...
def get_xinruiurl(file):
    ...
    print(">> found %d xinrui url" % len(urls))
    return urls
...
def check_file(file):
    ...
    for index, url in enumerate(urls):
        pbn_file=os.path.join(dir,"%s-%02d" % (filename, index + 1))
        xin2pbn(url.decode("utf-8"), pbn_file)
    # combine together
    outfilename = os.path.join(dir,filename + ".pbn")
    with open(outfilename, 'wb') as outfile:
        print("write all to one file %s" % outfilename)
        for index, url in enumerate(urls):
            pbn_file=os.path.join(dir,"%s-%02d.pbn" % (filename, index + 1))
            with open(pbn_file, 'rb') as readfile:
                shutil.copyfileobj(readfile, outfile)
    # remove files
    for index, url in enumerate(urls):
        pbn_file=os.path.join(dir,"%s-%02d.pbn" % (filename, index + 1))
        print("remove temple file %s" % pbn_file)
        if os.path.isfile(pbn_file):
            os.remove(pbn_file)

def generate_pbn_from_html(html_files):
    print(">> checking %s" % html_files)
