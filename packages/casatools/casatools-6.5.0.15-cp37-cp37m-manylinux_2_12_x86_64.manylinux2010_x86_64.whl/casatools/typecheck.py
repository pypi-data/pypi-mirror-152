...
class CasaValidator(Validator):
    ...
    def __is_floatvec(self, value):
        return isinstance(value,list) and all([self.__is_float(v) for v in value['value']]) or \
               isinstance(value,numpy.ndarray) and len(value.shape) <= 1 and \
               value.dtype.type in [ numpy.float32, numpy.float64, float ]
    ...
    def _validate_anyof(self, definitions, field, value):
        ...
        if valids < 1:
            super(CasaValidator, self)._error( _errors )
    ...
    def _validate_type_cReqPath(self,value):
        if isinstance(value,str) and os.path.exists(value):
            return True
        self._error(str(value),"path not found")

    def _validate_type_cPath(self,value):
        if isinstance(value,str):
            return True

    def _validate_type_cReqPathVec(self,value):
        if isinstance(value,list):
            if all([isinstance(v,str) and os.path.exists(v) for v in value]):
                return True
        self._error(repr(value),"path vector element not found")

    def _validate_type_cPathVec(self,value):
        if isinstance(value,list):
    ...
    def _validate_type_cBoolVec(self,value):
        if isinstance(value,list):
            if all([isinstance(v,bool) or isinstance(v,numpy.bool_) for v in value]):
                return True
        if isinstance(value,numpy.ndarray) and len(value.shape) <= 1:
