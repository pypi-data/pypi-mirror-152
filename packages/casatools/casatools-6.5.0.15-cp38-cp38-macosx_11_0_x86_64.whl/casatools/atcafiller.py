...
from .__casac__ import atcafiller as _atcafiller
...
class atcafiller:
    ...
    def open(self, msname='', filenames=[  ], options=[  ]):
        ...
        * noac : Do not load the autocorrelations
        """
        return self._swigobj.open(_str_ec(msname), _str_ec(filenames), _str_ec(options))
    ...
    def select(self, firstscan=int(0), lastscan=int(9999), spw=[ int(-1) ], lowfreq={'value': float(0.1), 'unit': 'GHz'}, highfreq={'value': float(999.0), 'unit': 'GHz'}, fields=[  ], edge=float(8.0)):
        ...
        """
        return self._swigobj.select(firstscan, lastscan, spw, _quant_ec(lowfreq), _quant_ec(highfreq), _str_ec(fields), edge)
