...
def exploration(csv, name_date_column, name_target_column, dict_columns_aggregate={}, window_start='',
                ...
                min_periods_corr=1, show_graph_correlation=True, min_correlation=0.6, plot_for_line=5, show_only_one='',
                constant_ols=True):
    # create csv
    ...
    if media_split:
        everything, media, organic, price, seasonality, no_media, no_organic, no_price, no_seasonality, media_google, media_fb, media_others = define_features(
            df=new_df, media_split=media_split)

        if show_graph_media:
            show_media(new_df, name_date_column, name_target_column, media, media_google, media_fb, media_others)
        if show_graph_no_media:
            show_no_media(new_df, name_date_column, name_target_column, organic, price, seasonality)

    else:
        everything, media, organic, price, seasonality, no_media, no_organic, no_price, no_seasonality = define_features(
            df=new_df, media_split=media_split)
        if show_graph_media:
