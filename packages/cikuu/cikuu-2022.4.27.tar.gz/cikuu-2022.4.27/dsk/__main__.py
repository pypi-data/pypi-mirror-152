...
class Dsk(object):
	...
	def json_to_dskmkf(self,infile, host='192.168.121.3',port=3306,user='root',password='cikuutest!',db='dskmkf', load=True):  
		...
		with open(infile.split('.')[0] + ".dsk", 'w') as fw: 
			for line in util.readline(infile) : #open(infile, 'r').readlines(): 
				try:
					arr = json.loads(line)
					essay = arr.get("essay", "") 
					if not essay: continue 
					dsk = localgec_todsk(essay, dskhost=self.dskhost)
					dsk['info'].update({"essay_id": arr.get("essay_id", 0), "rid": arr.get("request_id", 0), "uid": arr.get("user_id",0), "e_version": arr.get("version",0) })
					dsk.submit_dskmkf(dsk, cursor)	 #eid,rid,uid,ver = int( info.get('essay_id',0) ),int( info.get('rid',0) ),int( info.get('uid',0) ),int( info.get('e_version',0) )
					conn.commit()
	...
	def localgec_todsk(self, essay:str="She has ready. It are ok."): 
		''' test localgec todsk '''
		print (localgec_todsk(essay, dskhost=self.dskhost))
