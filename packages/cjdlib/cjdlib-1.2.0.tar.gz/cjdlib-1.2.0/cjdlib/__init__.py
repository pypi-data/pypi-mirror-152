...
def print_all(string: str, times: float = 0.1, line_feed: bool = False) -> None:
...
def log(func:function) -> any:
    ...
    def inner(*args, **kwargs):
        ...
        sys.stdout.write("--------------------\n")
        sys.stdout.flush()
        sys.stdout.write(f"{func.__name__}()函数运行完成，用时{round(end_time-start_time)}秒 {args} -> {res}\n按任意键继续...")
        sys.stdout.flush()
