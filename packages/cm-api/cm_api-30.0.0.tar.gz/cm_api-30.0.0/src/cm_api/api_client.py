...
class ApiResource(Resource):
  ...
  def __init__(self, server_host, server_port=None,
               ...
               ssl_context=None, timeout=None, override_base_url=None,
               auth_realm=API_AUTH_REALM, preemptive_auth=False):
    """
    ...
    @param override_base_url: Override base URL. Supercedes server_host, server_port,
                              and version arguments.
    @param auth_realm: Override the HTTP Basic Authentication realm.
    @param preemptive_auth: Preemptively send the HTTP Basic Authentication header
    ...
    if preemptive_auth:
      # Work around an issue where urllib2 only sends the HTTP Basic Auth header
      # if it gets a WWW-Authenticate: Basic. If SPNEGO is enabled, CM sends a
      # WWW-Authenticate: Negotiate, causing the API client to not authenticate.
    ...
    client.set_basic_auth(username, password, auth_realm)
