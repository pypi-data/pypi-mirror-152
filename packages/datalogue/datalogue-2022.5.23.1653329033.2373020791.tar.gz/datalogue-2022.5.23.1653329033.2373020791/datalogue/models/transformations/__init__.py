...
def _replace_label_from_payload(json: dict) -> Union[DtlError, ReplaceLabel]:
    ...
    if not isinstance(replacement, str):
        return DtlError(
            "replacement field is not a string in %s transformation"
            % ReplaceLabel.type_str
        )
...
class ReplaceValueByRegex(Transformation):
    ...
    def __init__(
        ...
        RegexTransformation.__init__(
            self, regex, regex_id, ReplaceValueByRegex.type_str
        )
        self.path = path
        self.regex = regex
...
class ReplaceLabelByRegex(RegexTransformation):
    ...
    def __init__(
        self, regex: Optional[str], regex_id: Optional[UUID], replacement: str
    ):
        RegexTransformation.__init__(
            self, regex, regex_id, ReplaceLabelByRegex.type_str
        )
        self.regex = regex
        self.regex_id = regex_id
        self.replacement = replacement
...
def _remove_empty_branches_from_payload(
    ...
    if json.get(Transformation.type_field) != RemoveEmptyBranches.type_str:
        return DtlError(
            "Dictionary input is not of type %s" % RemoveEmptyBranches.type_str
        )
...
class ExitField:
    ...
    def _as_payload(self) -> Union[dict, DtlError]:
        if self.children and self.data_type:
            return DtlError(
                "An exit field can have either a type (if child field) or children (if parent field)"
            )
        if not self.children and not self.data_type:
            return DtlError(
                "An exit field must have a type (if child field) or children (if parent field)"
            )
        if not isinstance(self.name, str):
            return DtlError("An exit field's name must be a string.")
        if self.data_type and not isinstance(self.data_type, DataType):
            return DtlError(
                "An exit field's datatype must be an enumeration of the DataType class."
            )
        ...
        if self.children and not isinstance(self.children, List):
            return DtlError(
                "An exit field's children must be a list of ExitField class(es)."
            )
        base = {"name": self.name}
        if self.data_type:
    ...
    def _from_payload(json: dict) -> Union[DtlError, "ExitField"]:
        ...
        if fields is None:
            ...
            if not isinstance(fields, List):
                return DtlError(
                    "An exit field's children should be a list of exit fields"
                )
        ...
        if data_type and not isinstance(data_type, DataType):
            return DtlError(
                "If an exit field has a data_type, it must be an enumeration of the DataType class."
            )
        if not isinstance(children, List):
            return DtlError(
                "If an exit field has children, it must be a list of ExitField(s)"
            )
...
class CustomCode(Transformation):
    ...
    def _as_payload(self) -> Union[dict, DtlError]:
        ...
        if not isinstance(self.language, Language):
            return DtlError(
                "Language must be a supported enumeration of the Language class"
            )
        base = {
            "type": "RunCode",
            "code": self.code,
    ...
    def _from_payload(json: dict) -> Union[DtlError, "CustomCode"]:
        ...
        if isinstance(exit_schema, DtlError):
            return exit_schema
        return CustomCode(
            code=code, exit_schema=exit_schema, language=Language.Javascript
        )
...
def _element_count_selection_from_payload(
    ...
    if json.get(Transformation.type_field) != ElementCountSelection.type_str:
        return DtlError(
            "Dictionary input is not of type %s" % ElementCountSelection.type_str
        )
...
class FilterByPathAndRegex(Transformation):
    ...
    def __init__(
        self, paths_and_regex: List[PathAndRegex], optional_input: bool = False
    ):
...
def _filter_by_path_and_regex_from_payload(
    ...
    if json.get(Transformation.type_field) != FilterByPathAndRegex.type_str:
        return DtlError(
            "Dictionary input is not of type %s" % FilterByPathAndRegex.type_str
        )
...
class InnerJoin(Transformation):
    ...
    def __repr__(self):
        return (
            ...
            + (
                f", sibling_to_field={self.sibling_to_field!r}, "
                if self.sibling_to_field is not None
                else ""
            )
            + (
                f", cache_source={self.cache_source!r}, "
    ...
    def _as_payload(self) -> dict:
        ...
        base["cacheTransformations"] = list(
            map(lambda t: t._as_payload(), self.cache_transformations)
        )
...
def _join_from_payload(
    json: dict, join_type: str
) -> Union[DtlError, OuterJoin, InnerJoin]:
    ...
    if isinstance(transformations_field, List):
        transformations_field = _parse_list(_transformation_from_payload)(
...
class ReplaceClass:
    ...
    def __eq__(self, other: "ReplaceClass"):
        if isinstance(self, other.__class__):
            return (
                self.threshold == other.threshold and self.new_class == other.new_class
            )
        return False
...
class FoldClassifications(Transformation):
    ...
    def _from_payload(json: dict) -> Union[DtlError, "FoldClassifications"]:
        if json.get(Transformation.type_field) != FoldClassifications.type_str:
            return DtlError(
                "Dictionary input is not of type %s" % FoldClassifications.type_str
            )
...
def _transformation_from_payload(json: dict) -> Union[DtlError, Transformation]:
    ...
    if type_field is None:
        return DtlError(
            "The json object doesn't have a '%s' property" % Transformation.type_field
        )

    parsing_function = _transformations.get(type_field)
    if parsing_function is None:
        return DtlError(
            "Looks like '%s' transformation is not handled by the SDK" % type_field
        )
