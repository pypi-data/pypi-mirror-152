...
def delete_nanfilled_lines(data, variables=False, inplace=False, count=True):
    ...
    if inplace:
      df_union.dropna(subset=variables, how="all", inplace=True)
      if count:
        print(f"\033[1m{datasize - df_union.shape[0]} lignes ont été supprimées.\033[0m")
    else:
      if count:
        print(f"\033[1m{datasize - df_union.shape[0]} lignes ont été supprimées.\033[0m")
      return df_union.dropna(subset=variables, how="all")
