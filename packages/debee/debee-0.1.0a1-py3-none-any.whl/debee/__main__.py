...
def _version_callback(value: bool) -> None:
    if value:
        typer.echo(
            f"{app.info.name} v.{__version__} -- visit https://bumblebee.freeforums.net/thread/6/debee-cli-related to chat about {app.info.name}."
        )
        raise typer.Exit()
...
def main(
    ...
    * debee file1 file2 -p  # show plots
    ...
    * debee file1 file2 -s
    ...
    if show_plot:
        try:
            ...
            logger.warning(
                " You need to install extras [plot] in order to use this option, e.g. pip install debee[plot], exiting..."
            )
