...
class CliTestCase(ReversibleTestCase, BaseTestCase):
    ...
    def invoke(self, *cli_blocks: str, bypass_error: bool = False, debug=False) -> Result:
    ...
    def _add_endpoint(self, endpoint_id: str, short_service_type: str, url: str):
        self.invoke('endpoints', 'add', '-t', short_service_type, endpoint_id)
        self.invoke('endpoints', 'set', endpoint_id, 'url', url)
        return self

    def _configure_endpoint(self, endpoint_id: str, config: Dict[str, str], debug=False):
        for k, v in config.items():
            self.invoke('endpoints', 'set', endpoint_id, k, v)
