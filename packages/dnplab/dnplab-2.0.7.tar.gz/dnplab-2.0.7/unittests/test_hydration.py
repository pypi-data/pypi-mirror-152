...
class TestHydration(unittest.TestCase):
    ...
    def test_hydration_return_dict(self):
        ...
        self.assertAlmostEqual(
            min(result["interpolated_T1"]), 2.051727288206873, places=6
        )
        self.assertAlmostEqual(max(result["interpolated_T1"]), 2.5383409868931706)
        self.assertEqual(len(result["ksigma_array"]), 21)
        self.assertAlmostEqual(
            min(result["ksigma_array"]), 2.5002212753387965, places=6
        )
        self.assertAlmostEqual(
            max(result["ksigma_array"]), 19.573744921860015, places=6
        )
        ...
        self.assertAlmostEqual(
            min(result["uncorrected_Ep"]), -2.90847384506599, places=6
        )
        self.assertAlmostEqual(
            max(result["uncorrected_Ep"]), 0.7434984782673344, places=6
        )
