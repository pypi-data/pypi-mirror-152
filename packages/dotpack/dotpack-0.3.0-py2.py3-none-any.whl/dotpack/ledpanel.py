...
class ledpanel:
    ...
    async def getimagedata(self,pilimage):
        if pilimage is None:
            sys.exit("Could not read the image.")
        #cv.imshow("Display window", img)
        dim=(16,16)
    ...
    async def fill_color(self,colorR: int,colorG: int,colorB: int):
        if colorR<=0xFF and colorR>=0 and colorG<=0xFF and colorG>=0 and colorB<=0xFF and colorB>=0 :
            if colorR<0x10:
                hexR=("0%x"%colorR)
            else:
                hexR=("%x"%colorR)
    ...
    async def draw_point(self,colorR: int,colorG: int,colorB: int, pointx: int, pointy: int):
        #check first
        if colorR<=0xFF and colorR>=0 and colorG<=0xFF and colorG>=0 and colorB<=0xFF and colorB>=0 and pointx >=0 and pointx<16 and pointy >=0 and pointy<16:
            if colorR<0x10:
                hexR=("0%x"%colorR)
            else:
                hexR=("%x"%colorR)
    ...
    async def tetris(self,game: int):
    ...
    async def savegif(self,gifname:str,picname:str):
        data = bytearray("$6 23|FS %s|%s"%(gifname,picname),'utf-8')
        await self.client.write_gatt_char(CHARACTERISTIC_UUID_RX, data)

    async def opengif(self,s:int,p:int,filename:str):
        data = bytearray("$6 24|FS %d %d|%s"%(s,p,filename),'utf-8')
        await self.client.write_gatt_char(CHARACTERISTIC_UUID_RX, data)

    async def opengif2(self):
        data = bytearray("$5 10",'utf-8')
        await self.client.write_gatt_char(CHARACTERISTIC_UUID_RX, data)
    
    async def displaygif(self,p:int):
        data = bytearray("$5 9 %d"%p,'utf-8')
        await self.client.write_gatt_char(CHARACTERISTIC_UUID_RX, data)
    
    async def displaygiffps(self,p:int):
        data = bytearray("$5 11 %d"%p,'utf-8')
        await self.client.write_gatt_char(CHARACTERISTIC_UUID_RX, data)

    async def getgifquantity(self,p:int,gifname:str):
        data = bytearray("$6 26|FS %d|%s"%(p,gifname),'utf-8')
        await self.client.write_gatt_char(CHARACTERISTIC_UUID_RX, data)

    async def delete_gif(self,p:int,filename:str):
        data = bytearray("$6 25|FS %d|%s"%(p,filename),'utf-8')
        await self.client.write_gatt_char(CHARACTERISTIC_UUID_RX, data)

    async def renameGIF(self,olefilename:str,newfilename:str):
        data = bytearray("$6 27|FS %s|%s"%(olefilename,newfilename),'utf-8')
        await self.client.write_gatt_char(CHARACTERISTIC_UUID_RX, data)

    async def gifsend(self):
        send = await self.client.read_gatt_char(CHARACTERISTIC_UUID_TX)
        str1 = str(send)
        str2 = "success!"
        print(str1)
        print(str1.find(str2))
        if (str1.find(str2) != -1):#15- .encode()
            print("Received")
            return True
        else:
            return False
    ...
    async def quantity_yes_or_no(self):
        send = await self.client.read_gatt_char(CHARACTERISTIC_UUID_TX)
        str1 = str(send)
        str2 = "success!"
        print(str1)
    ...
    async def flashsize(self):
        data = bytearray("$5 12",'utf-8')
        await self.client.write_gatt_char(CHARACTERISTIC_UUID_RX, data)

    async def filesize(self,directory:str,filename:str):
        data = bytearray("$6 31|FS %s %s"%(directory,filename),'utf-8')
        await self.client.write_gatt_char(CHARACTERISTIC_UUID_RX, data)

    async def folder_directory(self,directory:str):
        data = bytearray("$6 30|FS %s"%(directory),'utf-8')
        await self.client.write_gatt_char(CHARACTERISTIC_UUID_RX, data)

    async def dirname(self,directory:str):
        data = bytearray("$6 32|FS %s"%(directory),'utf-8')
        await self.client.write_gatt_char(CHARACTERISTIC_UUID_RX, data)

    async def deletedir(self,directory:str):
