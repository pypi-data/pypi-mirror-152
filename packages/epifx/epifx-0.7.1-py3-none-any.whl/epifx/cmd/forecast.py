...
def get_forecast_dates(all_obs, fs_from, fs_until, settings):
    ...
    valid_obs = [obs for obs in all_obs
                 if obs['date'] >= start and obs['date'] < until]
    ...
    fs_dates = sorted(obs['date'] for obs in valid_obs)
    if fs_from is None and fs_until is None:
        return [max(fs_dates)]
    if fs_from is not None:
        fs_dates = [d for d in fs_dates if d >= fs_from]
    if fs_until is not None:
        fs_dates = [d for d in fs_dates if d <= fs_until]
    return fs_dates
...
def run(ctx, forecast_dates, one_file=False, forecast_id=None):
    ...
    cache_file = filename_for_description(ctx.scenario_id, ctx.descriptor,
    ...
    if too_early:
        logger.warning("Excluding {} forecast dates prior to {}".format(
            len(too_early), start))
    too_late = [d for d in forecast_dates if d >= until]
    if too_late:
        logger.warning("Excluding {} forecast dates after {}".format(
            len(too_late), until))
    ...
    if one_file:
        ...
        state = pypfilt.forecast(ctx, forecast_dates, filename=fs_file)
    ...
    for fs_date in forecast_dates:
        ...
        if not any(fs_date == o['date'] for o in ctx.all_observations):
            ...
            if len(ctx.all_observations) > 0:
                ...
                logger.warning(msg.format(fs_date))
            # NOTE: setting `fs_date` to None has the following effects:
        ...
        pypfilt.build.set_chained(ctx.settings, ['epifx', 'forecast_id'],
                                  fs_str)

        if fs_date is None:
            state = pypfilt.fit(ctx, filename=fs_file)
        else:
            state = pypfilt.forecast(ctx, [fs_date], filename=fs_file)
        state['forecast_file'] = fs_file
...
def run_instance(instance, forecast_from, forecast_until, one_file, fs_id):
    ...
    fs_dates = get_forecast_dates(context.all_observations,
                                  forecast_from,
                                  forecast_until,
                                  context.settings)
    return run(context, fs_dates, one_file, forecast_id=fs_id)
...
def parser():
    ...
    fg.add_argument(
        '-u', '--until',
        help='Final forecasting date (YYYY-MM-DD)')
    fg.add_argument(
        '-o', '--one-output-file',
        action='store_true',
    ...
    fg.add_argument(
        '--id',
        help='Unique identifier for a single forecast')
...
def parse_date_arg(arg, fmt='%Y-%m-%d'):
    ...
    try:
        ...
        raise ValueError("Invalid forecast date '{}'".format(arg))
...
def main(args=None):
    ...
    instances = list(
        instance for instance in pypfilt.load_instances(args['config'])
        if (not scenario_ids)
        or instance.scenario_id in scenario_ids)

    # Warn about scenario IDs that were provided as command-line arguments but
    # were not found in the scenario files.
    if scenario_ids:
        for scenario_id in scenario_ids:
            matches = [instance for instance in instances
    ...
    if n_proc < 1:
        ...
        for instance in instances:
            run_instance(instance, forecast_from, forecast_until, one_file,
                         forecast_id)
