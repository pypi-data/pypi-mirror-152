...
class SEIR(Model):
    ...
    def field_types(self, ctx):
        return [(name, np.float_) for name in self.__fields]
    ...
    def update(self, ctx, step_date, dt, is_fs, prev, curr):
        ...
        if forcing_table is not None:
            ...
            force = forcing_table.lookup(step_date)
...
class SEEIIR(Model):
    ...
    def update(self, ctx, step_date, dt, is_fs, prev, curr):
        ...
        for col in cols_SEI:
            curr[col] = np.clip(curr[col], 0, 1)
        sum_SEI = curr['S'] + curr['E1'] + curr['E2'] + curr['I1'] + curr['I2']
        mask_invalid = sum_SEI > 1
        if np.any(mask_invalid):
            denom = sum_SEI[mask_invalid]
            for col in cols_SEI:
                curr[col][mask_invalid] = (curr[col][mask_invalid] / denom)
            # Update the net non-R population.
            sum_SEI = (curr['S'] + curr['E1'] + curr['E2']
                       + curr['I1'] + curr['I2'])
...
class NoEpidemic(Model):
    ...
    def pr_inf(self, prev, curr):
        return np.zeros(curr.shape, dtype=np.float_)
