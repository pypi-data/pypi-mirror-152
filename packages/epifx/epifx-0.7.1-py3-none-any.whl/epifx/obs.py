...
class SampleFraction(pypfilt.obs.Univariate):
    ...
    * ``k_obs_field``: The name of the state vector field that contains the
    ...
    def distribution(self, ctx, snapshot, ixs=None):
        ...
        """
        return self._dist_frac(ctx, snapshot, ixs=ixs)

    def log_llhd(self, ctx, snapshot, obs, ixs=None):
        """
        ...
        dist = self._dist_count(ctx, snapshot, denom, ixs=ixs)
        return dist.logpmf(num)

    def simulate(self, ctx, snapshot, rng, ixs=None):
        """
        Simulate the case fraction with respect to the default denominator.
        """
        dist = self._dist_count(ctx, snapshot, self.denom, ixs=ixs)
        # NOTE: ensure we always return an array.
        return np.atleast_1d(dist.rvs(random_state=rng)) / self.denom

    def simulated_obs(self, ctx, snapshot, rng, ixs=None):
        fracs = self.simulate(ctx, snapshot, rng, ixs=ixs)
        return [{
            'date': snapshot.time,
            'value': frac,
    ...
    def simulated_field_types(self, ctx):
        cols = [time_field('date'),
                ('numerator', np.int32),
                ('denominator', np.int32)]
        return cols

    def _dist_count(self, ctx, snapshot, denom, ixs=None):
        """
        Return the case count distribution for each particle, for a known
        denominator.
        """
        mean_pr = self._mean(ctx, snapshot)
        op = ctx.settings['observations'][self.unit]['parameters']
    ...
    def _dist_frac(self, ctx, snapshot, ixs=None):
        """
        Return the case fraction distribution for each particle.
        """
        mean_pr = self._mean(ctx, snapshot)
    ...
    def _mean(self, ctx, snapshot):
        ...
        pr_inf = ctx.component['model'].pr_inf(prev['state_vec'],
                                               curr['state_vec'])
    ...
    def _disp(self, mu, op, denom):
        ...
        if 'bg_var' in op and op['bg_var'] > 0:
            ...
            min_var = op['bg_var']
            frac_var = (mu * (1 - mu) * (1 + (denom - 1) / (disp + 1))
                        / denom)
    ...
    def from_file(self, filename, time_scale, year=None, time_col='to',
        ...
        keep_columns = ['date', 'numerator', 'denominator']
        ...
        fields = [time_field('date'), ('numerator', np.int32),
                  ('denominator', np.int32)]
    ...
    def obs_into_row(self, obs, dtype):
        return (obs['date'], obs['numerator'], obs['denominator'])
...
class PopnCounts(pypfilt.obs.Univariate):
    ...
    * ``pr_obs_field``: The name of the state vector field that contains the
    ...
    def distribution(self, ctx, snapshot, ixs=None):
        ...
        nb_pr = nb_k / (nb_k + mu)
        if ixs is not None:
            if isinstance(nb_k, np.ndarray) and nb_k.shape == nb_pr.shape:
                nb_k = nb_k[ixs]
            nb_pr = nb_pr[ixs]
    ...
    def _mean(self, ctx, snapshot):
        ...
        if self.pr_obs_lookup is not None:
            ...
            if table.value_count() == 1:
                ...
                msg_fmt = 'No lookup indices for table {} and {} observations'
                raise ValueError(msg_fmt.format(self.pr_obs_lookup, self.unit))
