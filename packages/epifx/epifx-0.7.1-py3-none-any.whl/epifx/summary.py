...
class PrOutbreak(Table):
    ...
    def field_types(self, ctx, obs_list, name):
        ...
        date = time_field('date')
        return [date, ('pr', np.float64)]

    def n_rows(self, ctx, start_date, end_date, n_days, forecasting):
        return n_days

    def add_rows(self, ctx, fs_date, window, insert_fn):
        for snapshot in window:
            mask = self.__model.is_seeded(snapshot.vec['state_vec'])
...
class PeakMonitor(Monitor):
    ...
    **current simulation window**.
    ...
    def begin_sim(self, ctx, start_date, end_date, n_days, forecasting):
    ...
    def load_state(self, ctx, grp):
        ...
        for unit in self.__obs_units:
            ...
            self.peak_date[unit] = np.array([time_scale.from_dtype(d)
                                             for d in peak_date])
...
class ThresholdMonitor(Monitor):
    ...
    exceeded the specific threshold: ``exceed_date[unit]``.
    ...
    def prepare(self, ctx, obs_list, name):
        threshold = ctx.get_setting(
            ['summary', 'monitors', name, 'threshold'],
            self.__threshold)
        ...
        for snapshot in window:
            ...
            for unit in self.__obs_units:
                ...
                mask = np.logical_and(
                    ...
                    ~ self.exceed_mask[unit])
                self.exceed_date[unit][mask] = snapshot.time
    ...
    def load_state(self, ctx, grp):
        ...
        for unit in self.__obs_units:
            ...
            self.exceed_date[unit] = np.array([time_scale.from_dtype(d)
    ...
    def save_state(self, ctx, grp):
        ...
        for unit in self.__obs_units:
            ...
            exceed_date = np.array([time_scale.to_dtype(d)
                                    for d in self.exceed_date[unit]],
                                   dtype=time_dtype)
    ...
    * ``bin_width``: the width of the time bins.
    ...
    def __apply_settings(self, ctx, name):
        ...
        self.__fs_only = ctx.get_setting(
            ['summary', 'tables', name, 'only_forecasts'],
            False)
    ...
    def field_types(self, ctx, obs_list, name):
        ...
        fs_date = time_field('fs_date')
        week_start = time_field('week_start')
        prob = ('prob', np.float64)
        return [unit, fs_date, week_start, prob]
    ...
    def finished(self, ctx, fs_date, window, insert_fn):
        for unit in self.__obs_units:
            dates = self.__monitor.exceed_date[unit]
            ...
            for (wk_start, wk_end) in self.__bins:
                ...
                row = (unit, fs_date, wk_start, prob)
                insert_fn(row)
...
class PeakForecastEnsembles(Table):
    ...
    * ``only_forecasts``: whether to record results only during forecasts.
    ...
    def field_types(self, ctx, obs_list, name):
        ...
        return [unit, fs_date, weight, date, value]
...
class PeakForecastCIs(Table):
    ...
    * ``credible_intervals``: the central credible intervals to record; the
    ...
    def field_types(self, ctx, obs_list, name):
        ...
        return [unit, fs_date, prob, s_min, s_max, t_min, t_max]
    ...
    def finished(self, ctx, fs_date, window, insert_fn):
        ...
        for unit in self.__obs_units:
            ...
            # Convert from days after the forecast date to time values.
            def time(days):
                """Convert peak times from days (as measured from the
                forecast date)."""
                return time_scale.add_scalar(fs_date, days)

            for pctl in self.__probs:
                row = (unit, fs_date, pctl, sz_ints[pctl][0],
                       sz_ints[pctl][1], time(tm_ints[pctl][0]),
                       time(tm_ints[pctl][1]))
                insert_fn(row)
    ...
    * ``thresholds``: the accuracy thresholds for peak size predictions,
    ...
    def field_types(self, ctx, obs_list, name):
        ...
        if self.__monitor_name is None:
            msg_fmt = 'Table {} has no peak_monitor'
            raise ValueError(msg_fmt.format(name))
        self.__toln = np.uint8(ctx.get_setting(
            ['summary', 'tables', name, 'thresholds'],
            self.__toln))
        self.__num_toln = len(self.__toln)
        self.__monitor = ctx.component['summary_monitor'][self.__monitor_name]
        self.__all_obs = obs_list
        ...
        var = ('var', np.float64)
        savg = ('avg', np.float64)
        return [unit, fs_date, toln, acc, var, savg]
    ...
    def finished(self, ctx, fs_date, window, insert_fn):
        ...
        for unit in self.__obs_units:
            ...
            for pcnt in self.__toln:
                ...
                row = (unit, fs_date, pcnt, accuracy, sop_var, sop_avg)
                insert_fn(row)
    ...
    * ``thresholds``: the accuracy thresholds for peak time predictions,
    ...
    def field_types(self, ctx, obs_list, name):
        ...
        if self.__monitor_name is None:
            msg_fmt = 'Table {} has no peak_monitor'
            raise ValueError(msg_fmt.format(name))
        self.__toln = np.array(ctx.get_setting(
            ['summary', 'tables', name, 'thresholds'],
            self.__toln))
        self.__num_toln = len(self.__toln)
        self.__monitor = ctx.component['summary_monitor'][self.__monitor_name]
        ...
        var = ('var', np.float64)
        tavg = time_field('avg')
        return [unit, fs_date, toln, acc, var, tavg]
    ...
    def finished(self, ctx, fs_date, window, insert_fn):
        ...
        for unit in self.__obs_units:
            ...
            top_avg = ctx.component['time'].add_scalar(fs_date, dtp_avg)
            ...
            for days in self.__toln:
                ...
                row = (unit, fs_date, days, accuracy, dtp_var, top_avg)
                insert_fn(row)
...
class ExpectedObs(Table):
    ...
    def field_types(self, ctx, obs_list, name):
        self.__probs = np.array(ctx.get_setting(
            ['summary', 'tables', name, 'credible_intervals'],
            self.__probs))
        self.__obs_units = sorted(ctx.component['obs'].keys())
        unit = string_field('unit')
        ...
        return [unit, fs_date, date, prob, ymin, ymax]

    def n_rows(self, ctx, start_date, end_date, n_days, forecasting):
        # Need a row for each interval, for each day, for each data source.
        n_obs_models = len(ctx.component['obs'])
        return len(self.__probs) * n_days * n_obs_models
    ...
    def add_rows(self, ctx, fs_date, window, insert_fn):
        for unit in self.__obs_units:
            ...
            for snapshot in window:
                ...
                cinfs = stats.cred_wt(expected, snapshot.weights,
                                      self.__probs)
                for cix, pctl in enumerate(self.__probs):
                    row = (unit, fs_date, snapshot.time, pctl,
...
class ObsLikelihood(Table):
    ...
    def load_state(self, ctx, group):
        ...
        pypfilt.cache.load_rng_states(group, 'prng_states', {
            'resample': self.__rnd,
        })

    def save_state(self, ctx, group):
        """Save the current state of each PRNG to the cache."""
        pypfilt.cache.save_rng_states(group, 'prng_states', {
            'resample': self.__rnd,
        })
    ...
    def field_types(self, ctx, obs_list, name):
        ...
        ctx.install_event_handler('log_llhd', self.record_obs_llhd)
        fs_date = time_field('fs_date')
        date = time_field('date')
        value = ('value', np.float64)
        ...
        unit = string_field('unit')
        return [fs_date, unit, date, value, llhd, std_err, forecast]
    ...
    def add_rows(self, ctx, fs_date, window, insert_fn):
        ...
        for snapshot in window:
            ...
            if snapshot.time <= self.__start_date:
                continue
            # Identify observations for this date that are not being filtered.
            obs = [o for o in self.__obs_tbl.get(snapshot.time, [])
    ...
    def record_obs_llhd(self, obs, log_llhds, weights):
        ...
        row = (self.__fs_date, obs['unit'], obs['date'],
