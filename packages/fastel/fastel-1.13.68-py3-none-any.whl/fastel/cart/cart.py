...
class Cart(BaseCart):
    ...
    def _calc_coupon_discounts(
        ...
        # personal coupon
        if assigned_user_id:
            if assigned_user_id == self._cache_cart["owner"]:
                items.append(
                    {
                        "type": "coupon",
                        "name": coupon["name"],
            ...
            return items
        # global coupon (user can use only one time)
        else:
            if self._cache_cart["owner"] in used_users:
                raise CartException("coupon_is_used")
            items.append(
                {
                    "type": "coupon",
                    "name": coupon["name"],
