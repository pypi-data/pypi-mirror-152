...
from functions_framework.exceptions import (
    ...
    MissingFunctionContextENV,
    MissingOFFunctionInputENV,
    InvalidFuncContextInputType,
    InvalidFuncContextOutputType
)
from google.cloud.functions.context import Context
from functions_framework._of.v2.context import FunctionContext
_FUNC_CTX = dict()
...
# fc add of function
def openfunctin_event(func):
    """Decorator that registers openfunction event as user function signature type."""
    _function_registry.REGISTRY_MAP[
        func.__name__
    ] = _function_registry.OF_SIGNATURE_TYPE
...
def create_dapr_app(target=None, source=None, signature_type=None, func_context=None):
    target = _function_registry.get_function_target(target)
    source = _function_registry.get_function_source(source)
    ...
    _configure_dapr_app(_app, function, func_context)
    return _app


def _configure_dapr_app(_app, function, ctx: FunctionContext):
    # 1 check input and create dapr app
    # {'uri': 'cron', 'component': 'serving-r4j4z-component-cron-s2ssr', 'type': 'bindings'}
    inputs = ctx.inputs
    if inputs is not None:
        for input in inputs.values():
            if input.type == _DAPR_BINDINGS:
                _app.binding(input.component)(function)
                print("binding {}".format(input.component))
            elif input.type == _DAPR_PUBSUB:
                _app.subscribe(topic=input.uri, pubsub_name=input.component)(function)
                print("sub topic {}, pubsub name {}".format(input.uri, input.component))
            else:
                raise InvalidFuncContextInputType(
                    "Invalid input type {}".format(input.type)
                )
    else:
        raise MissingOFFunctionInputENV(
            "don't find inputs in FUNC_CONTEXT"
        )
    # 2 check output
...
def _parse_function_context(ctx):
    dic_ctx = json.loads(ctx)

    _ctx = FunctionContext(**dic_ctx)

    return _ctx


def parse_function_context():
    ctx = os.getenv("FUNC_CONTEXT", "")
    if ctx != "":
        return _parse_function_context(ctx)
    else:
        raise MissingFunctionContextENV(
            "don't find env value FUNCTION_CONTEXT"
        )
        ...
        if not self.app:
            ...
            if  ff == "":
                ...
                if _ctx.runtime == KNATIVE_FUNC:
                    ...
                    self.app = create_dapr_app(self.target, self.source, self.signature_type, _ctx)
