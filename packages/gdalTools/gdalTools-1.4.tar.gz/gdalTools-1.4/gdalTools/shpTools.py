...
import rasterio
from osgeo import ogr, gdal, gdalnumeric, gdal_array, osr
...
def remove_ship_like_features(inPath, outPath,
                              threshold_r1=5, threshold_r2=6, threshold_a=2000):
    """
        This function is used to remove some long and narrow features in shapefile.
        :param inPath: the path of input shapefile
        :param outPath: the path of output shapefile
        :return: None
    """
    driver = ogr.GetDriverByName("ESRI Shapefile")
    ...
    out_lyr = out_ds.CreateLayer(outPath, layer.GetSpatialRef(), ogr.wkbPolygon)
    def_feature = out_lyr.GetLayerDefn()
    # 遍历原始的Shapefile文件给每个Geometry做Buffer操作
    ...
    for feature in layer:
        ...
        ratio1 = max(xmax - xmin, ymax - ymin) / min(xmax - xmin, ymax - ymin)
        if ratio1 > threshold_r1:
            continue

        area_in_sq_m = geom2.GetArea()

        geom2.Transform(transform)
        area_c = geom2.GetArea()
        ratio2 = area_b / area_in_sq_m
        print(ratio2)
        if ratio2 > threshold_r2 and area_c < threshold_a:
            continue
