...
def GetGeoImgClips(imgRoot, outRoot, targetSize=576, PaddingSize=64, ImgSuffix='.tif'):
    imglist = glob.glob(f"{imgRoot}/*.tif")
    # labels_list = ['./data/train2.tif']

    imgs_num = len(imglist)
    print("imgs_num:{}".format(imglist))
    mkdir(outRoot)

    for k in range(imgs_num):
        im_proj, im_geotrans, im_width, im_height, image = read_img(imglist[k])
        image = np.transpose(image, (1, 2, 0))

        imgName = os.path.split(imglist[k])[-1].split('.')[0]
        ...
        for i in range(rows):
            for j in range(cols):
                ...
                start_point = computerStartPoint(i, j, targetSize, PaddingSize, height, width)
                ...
                try:
                    geoClip(imglist[k], tempPath, start_point, size=size)
                    subImg_num += 1
                except:
                    continue


def computerStartPoint(i, j, targetSize, PaddingSize, height, width):
...
def geoClip(imgPath, outPath, start_point, size=640):
