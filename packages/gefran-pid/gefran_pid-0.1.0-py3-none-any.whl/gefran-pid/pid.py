...
class Pid(Runnable):
    ...
    def __init__(self,
        ...
        :param int, baudrate : Baudrate set in the PID controller for communication. Default: 9600.
    ...
    def set_up_data(self):
        ...
        self.set_up_temp_data(start_time=start_time)
    ...
    def disconnect(self):
        ...
        if self.modbus is not None:
            try:
                # TODO: implement: close connection: stop monitoring, to safe state? save data to disk
                # disconnect
                self.modbus.close()
            except Exception as e:
    ...
    def read_setpoint(self) -> float:
        return self.modbus.execute(self._controller_id, cst.READ_INPUT_REGISTERS, Pid.ADDRESS_SP, 1, data_format='>e')

    def write_setpoint(self, value):
        self.modbus.execute(self._controller_id, cst.WRITE_MULTIPLE_REGISTERS, Pid.ADDRESS_SP, output_value=[value],
                            data_format='>e')
        command_str = Pid.Commands.set_setpoint + f' to {value:.2f}°C'
        self.register_command(command_str)

    def read_process_value(self) -> float:
        return self.modbus.execute(self._controller_id, cst.READ_INPUT_REGISTERS, Pid.ADDRESS_PV, 1, data_format='>e')
    ...
    def set_up_temp_data(self, start_time) -> None:
        """
    ...
    def wait_until_stable(self,
        ...
        :param int, n: Number of logged data points that is used for determining if the temperature is
        stable.
        :param float, time_out: Stop the waiting period, even if no stable temperature has been
        obtained after this number of seconds.
        :param custom_stable_func: Provide a custom function for
        determining if the temperature is stable (fun(times: List[float], temps: List[float]) -> bool,
        times are expressed in seconds). If None is set, a built-in function will be used which checks
        if each of the last n data points are within a temperature tolerance interval
        (setpoint - tolerance) < data point < setpoint + tolerance).
        :param float, tolerance: Allowed temperature tolerance (in °C) for obtaining a stable temperature if
        the default stable function is used.
