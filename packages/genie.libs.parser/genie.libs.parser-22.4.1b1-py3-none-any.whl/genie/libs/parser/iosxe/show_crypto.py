...
class ShowCryptoGdoiFeature(ShowCryptoGdoiFeatureSchema):
    ...
    def cli(self, output=None):
        ...
        # Group Name: getvpn1
        p1 = re.compile(r'^Group Name:\s(?P<group>[\w\d]+)$')

        #     Key Server ID       Version   Feature Supported
        p2 = re.compile(r'Key Server ID \s+Version\s+Feature\sSupported')

        #         1.1.1.1          1.0.27         Yes
        p3 = re.compile(r'(?P<key_server_id>\d\.\d\.\d\.\d)\s+(?P<key_version>[\d\.]+)\s+(?P<key_feature_supported>[\w]+)$')

        #    Group Member ID     Version   Feature Supported
        p4 = re.compile(r'Group Member ID\s+Version\s+Feature\sSupported')

        #         3.3.1.1          1.0.26         Yes
        p5 = re.compile(r'(?P<group_member_id>\d\.\d\.\d\.\d)\s+(?P<group_member_version>[\d\.]+)\s+(?P<group_feature_supported>[\w]+)$')
        ...
        for line in output.splitlines():
            ...
            if m:
                group = m.groupdict()
                if count == 1:
                    server_dict.update({'key_server_id': group['key_server_id']})
                    server_dict.update({'key_version': group['key_version']})
                    server_dict.update({'key_feature_supported': group['key_feature_supported']})
                    count += 1
                    continue
