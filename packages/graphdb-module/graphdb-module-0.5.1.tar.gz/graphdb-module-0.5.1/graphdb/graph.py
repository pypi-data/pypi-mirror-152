...
class GraphDb(
    ...
    def custom_query(
        ...
        if (
            ...
            or any(p.lower() == "drop" for p in payload.values())
        ):
    ...
    @staticmethod
    def _chunks(lst: List[Node], n: int) -> Iterator[List[Node]]:
    ...
    def create_multi_node(self, nodes: List[Node]) -> bool:
        ...
        """
        # define maximum chunking value
        max_chunking_value = int(os.getenv("MAX_CHUNKS", 100))
        # define new queue for chunk value
        q = multiprocessing.Queue()
        # define multiprocessing pool
        pool = multiprocessing.Pool(
            processes=int(os.getenv("MAX_WORKER_MULTIPROCESSING", 10)),
            initializer=_create_multi_node,
            initargs=(q,),
        )
        # adding all chunking value to queue
        for data in self._chunks(nodes, max_chunking_value):
            q.put(
                (
                    data,
                    self.connection.connection_uri_reader,
                    self.connection.connection_uri_writer,
                )
            )

        # wait queue to empty and all process is done
        q.close()
        q.join_thread()

        # close process and join all subprocess
        pool.close()
        pool.join()
        return True
    ...
    def update_relationship_property(
        ...
        # node relationship
        n_rel = (
            self.connection.driver_writer.V(n_f).outE().hasLabel(rel.relationship_name)
        )
...
def _create_multi_node(q: multiprocessing.Queue) -> None:
    """Inserting multiple nodes using multi processing
    :param q: tuple of queue that will executed by worker
    :return: none
    """
    while not q.empty():
        # get data from queue
        nodes, reader_uri, writer_uri = q.get()
        # give some verbose message
        print("worker {} is executing {} task".format(os.getpid(), len(nodes)))
        # initialize different connection for each workers
        conn = GraphDbConnection.from_uri(writer_uri, reader_uri)
        # define new graph object based on this connection
        g = GraphDb.from_connection(conn).connection.driver_writer
        ...
        # save to database
        g.iterate()
        # close connection after done process
        conn.close()

    print("worker {} is done executing {} task".format(os.getpid(), max_chunking_value))
