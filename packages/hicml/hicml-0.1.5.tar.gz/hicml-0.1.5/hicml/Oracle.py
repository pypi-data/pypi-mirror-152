...
class Oracle:
    def __init__(self, loop_list_path: str,
                 ...
                 width: int = 56, stem:str, num_straw_workers:int, chr_list: list = None):
 ...
 def makePredictions(self,chr):
    scores=[]
    df_tmp = self.loopList[self.loopList['chr1']==chr]
    df_tmp = df_tmp.reset_index(drop=True)
    batch = np.zeros((len(df_tmp),self.__width,self.__width,1))
    for reso in self.__resolutions:
      print ('ORACLE is running at', reso, 'kb', 'on chr', chr)
      mzd = self.__hic.getMatrixZoomData(chr, chr, "observed", self.__norm , "BP", reso)
      for i, row in df_tmp.iterrows():   
        x1,x2,y1,y2 = self.makeRegion((row['chr1'],row['x1'],row['x2'],row['y1'],row['y2']),self.__width-1,reso)
    ...
    def run(self):
        ...
        with ThreadPoolExecutor(max_workers = self.__numWorkers) as executor:
          results = executor.map(self.makePredictions, self.__chr)
          for r in results:
            print (r)
          print('All Oracle jobs submitted', flush=True)
          df = pd.concat(self.__oracleList,ignore_index=True).fillna('.')
          df.to_csv(self.__stem+'_ORACLE_merged.bedpe', sep = "\t", index=False)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 8:
        ...
        CHR_LIST = sys.argv[7].split(',')

    oracle = Oracle(BEDPE_DIR, FILEPATH, MODEL_DIR, NORMALIZATION_TYPE, STEM, NUM_WORKERS, chr_list=CHR_LIST)
