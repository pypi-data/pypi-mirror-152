...
def create_fig(
    ...
    fig = make_subplots(
        rows=rows_num, cols=1, shared_xaxes=False, vertical_spacing=vertical_spacing, row_width=row_width
    )

    for metric, metric_df in df.groupby("Metric"):
        metric_idx = metrics.index(metric)
        for ticker, ticker_df in metric_df.sort_values(["Period"]).groupby("Ticker"):
...
def plot(
    ...
    with console.status("Fetching data...", spinner="material") as status:
        ...
        cmd_context["plot_title"] = f"Historical daily {' and '.join(list(set(df['Metric'])))}"
        status.update("Generating plot...")
        fig = create_fig(df, cmd_context, tickers_list)
