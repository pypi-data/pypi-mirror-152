...
class ExperimentIterableDataset(IterableDataset):
    ...
    def __init__(
        ...
        sample_size_limit: int = 1073741824,  # 1GB in Bytes is verified later by _set_sample_size_limit,
    ...
    def _set_sample_size_limit_(self):
        ...
        size_mem = int(re.findall(r'[0-9]+', os.environ.get('MEM', '32'))[0])
        if size_mem >= 32:
            self.sample_size_limit = 1073741824     # 1GB in bytes
        else:
            self.sample_size_limit = 104857600      # 100MB in bytes
