        ...
        if not self._client.WSD:
            ...
            if lang == 'R':
                rscript_sw_spec_ids = [self._client.software_specifications.get_id_by_name(sw_name)
                                       for sw_name in ('default_r3.6', 'runtime-22.1-r3.6')]

                if script_meta[u'software_spec_uid'] not in rscript_sw_spec_ids:
                    raise WMLClientError("For R scripts, only base software specs 'default_r3.6', 'runtime-22.1-r3.6' "
