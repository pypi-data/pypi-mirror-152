...
async def present(
    ...
    **Autogenerated function**
    ...
    if resource_id:
        before = await hub.exec.boto3.client.ec2.describe_vpcs(
            ctx, VpcIds=[resource_id]
        )
        if not before["result"]:
            if "InvalidVpcID.NotFound" not in str(before["comment"]):
                result["comment"] = before["comment"]
                result["result"] = False
            return result
    if before and before["ret"].get("Vpcs"):
        result[
            "old_state"
        ] = await hub.tool.aws.ec2.conversion_utils.convert_raw_vpc_to_present(
            ctx, raw_resource=before["ret"]["Vpcs"][0], idem_resource_name=name
        )
        ...
        ret = await hub.exec.boto3.client.ec2.create_vpc(
            ...
            TagSpecifications=[{"ResourceType": "vpc", "Tags": tags}] if tags else None,
            **cidr_request_payload,
        )
    ...
    try:
        if ctx.get("test", False):
            ...
            after = await hub.exec.boto3.client.ec2.describe_vpcs(
                ctx, VpcIds=[resource_id]
            )
            if not after["result"]:
                result["result"] = False
            ...
            result[
                ...
                ctx, raw_resource=after["ret"]["Vpcs"][0], idem_resource_name=name
            )
        else:
...
async def absent(
    ...
    **Autogenerated function**
    ...
    if not resource_id:
        ...
        return result
    before = await hub.exec.boto3.client.ec2.describe_vpcs(ctx, VpcIds=[resource_id])
    if not before["result"]:
        if "InvalidVpcID.NotFound" in str(before["comment"]):
            result["comment"] = hub.tool.aws.comment_utils.already_absent_comment(
                resource_type="aws.ec2.vpc", name=name
            )
        else:
...
async def search(
    ...
    if not ret["result"]:
        ...
        return result
    if not ret["ret"]["Vpcs"]:
        result["comment"] = (f"Unable to find aws.ec2.vpc '{name}'",)
        return result
