...
class ConsulService(object):
    ...
    def registerAPI(self, local_ip, port, app_name, app_id, prefix, app_group, instance_info, apis, protocol='http',
        ...
        for api in apis:
            uid = str(uuid.uuid4())
            suid = ''.join(uid.split("-"))
            prefix = '{}.http.routers.{}'.format(prefix, suid)
            tags.append("{}.rule=Path(`{}`) && Method(`{}`)".format(prefix, api['path'], api['method']))
            key = None
            if 'scope' in api.keys():
                ...
                key = 'scopes'
            if key is not None:
                tags.append("{}.middleware=nezha@interval".format(prefix))
                tags.extend(["{}.metadata.nezha.scopes={}".format(prefix, scope) for scope in api[key].split(',')])
...
if __name__ == '__main__':
    ...
    api_entries = [
        ...
        },
        {
            ...
            "method": "GET",
            "scopes": "mes:aging:get,mes:agi:get"
        }
    ]
