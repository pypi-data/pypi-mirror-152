...
def _error_handling(pvt_table, pressure, temperature):
...
def retrieve_fluid_properties(
    ...
    Function to obtain value for selected fluid property (`param`) corresponding to the pressure/temp pairs input as series. The input fluid file type is a .tab file that is an output
    of an equation of state simulator (i.e PVTSim, MultiFlash) and is an input file specifically for OLGA. There are 31 fluid property parameters included in the .tab file which are the
    options as an output for this function. Note that pressure and temperature sensors have to come from the same location.

    Args:
        pressure: Pressure time series
            Pressure sensor time series.
        temperature: Temperature time series
            Temperature sensor time series.
        pvt_data: PVT data
            This is a PVT table that has been parse out from a .tab file which is a standardized generated output of
    ...
    param_idx = [idx for idx, val in enumerate(column_names) if param == val][0]
