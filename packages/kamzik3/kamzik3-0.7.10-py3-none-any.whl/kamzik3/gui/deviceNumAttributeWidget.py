...
from kamzik3.constants import ATTR_STATUS, READY_DEVICE_STATUSES, STATUS_BUSY
...
class DeviceNumAttributeWidget(Ui_Form, DeviceWidget, StackableWidget):
    ...
    def __init__(
        ...
        self.value_controls: List[QWidget] = []
    ...
    def setupUi(self, Form):
        ...
        self.input_step_size.setProperty("value", self.step_size)
    ...
    def _check_unit(self, factor):
        """
        Check and format the step size depending on its unit.

        For degC one has to convert it to delta_degC.
        see https://pint.readthedocs.io/en/stable/nonmult.html
        """
        step_size = self.input_step_size.value()
        if self.input_value.get_unit() != "degC":
            delta = units.Quantity(step_size, self.input_value.get_unit())
        else:
            delta = units.Quantity(step_size, "delta_degC")
        return factor * delta

    @pyqtSlot()
    def decrease_value(self):
        return self.input_value - self._check_unit(factor=1)

    @pyqtSlot()
    def decrease_value_double(self):
        return self.input_value - self._check_unit(factor=10)
...
class DeviceNumAttributeBusyWidget(DeviceNumAttributeWidget):
    """
    Same class as DeviceNumAttributeWidget except that the widget is enabled
    only when the device is busy.
    """
