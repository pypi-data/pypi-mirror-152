...
def get_search_results(query):  
        try:
            ...
            for links in source_url:
                ...
                res_list_search.append({"title":f"{title}","mangaid":f"{mangaid}"})
            if res_search_list == []:
...
def get_manga_details(mangaid):  
        try:
            ...
            for i in manSplit:
                ...
                viewk = ", ".join(view)
                authork = ", ".join(author)
                res_search_list = {"title":f"{title}","description":f"{description}","image":f"{imagerl}","status":f"{statusk}","view":f"{viewk}","author":f"{authork}","alternative":f"{alternativek}","genre":f"{genrek}"}          
            return res_search_list
