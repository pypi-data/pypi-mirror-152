...
class ShareHandler(BaseHandler):
    ...
    async def get(self, name):
        if self.app.options.auth:
            ...
            query = {'path': {'$regex': re.compile(f'^{name}/')}}
            docs = await self.query('share', query)
            entries = []
            for doc in docs:
...
class DiskHandler(tornado.web.StaticFileHandler, BaseHandler):
    ...
    async def post(self, name):
        ...
        if not path.exists() and self.args.action and self.args.action not in ['unshare', 'merge']:
            ...
            doc = await self.db.share.find_one({'token': self.current_user.token, 'path': name})
            if doc and self.args.batch:
                ...
                if self.args.days and self.args.days != '0':
                    doc['expired_at'] = doc['created_at'] + datetime.timedelta(days=int(self.args.days)) - datetime.timedelta(hours=8)
                doc = await self.db.share.find_one_and_update({'token': self.current_user.token, 'path': name},
                                                              {'$set': doc},
                                                              upsert=True,
            ...
            if not self.app.options.auth:
                ...
                await self.db.share.delete_one({'token': self.current_user.token, 'path': name})
                self.finish({'err': 0, 'msg': f'{name}已取消分享'})
