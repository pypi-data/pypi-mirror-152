...
from lettuce import build_failed_first_case
...
def print_end(total=None):
    ...
    if isinstance(total, core.SummaryTotalResults):
        ...
        color = "\033[1;32m"
        if total.features_passed_overall == 0:
            color = "\033[0;31m"
    ...
    if total.failed_scenario_locations:
        ...
        wrt("\033[0m")
        wrt("\n")
    # build_failed_first_case()
    try:
        build_failed_first_case()
    except Exception as e:
        print(f'colored_shell_output.py ::build_failed_first_case()出错了，原因：{e}')
