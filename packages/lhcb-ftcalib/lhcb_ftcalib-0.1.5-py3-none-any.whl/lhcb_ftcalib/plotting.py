...
def calibration_lineshape(tagger, eta_plot):
    """ Returns the lineshape of the calibration curve
        ...
        :return type: list
    """
    return tagger.func.eval_plotting(tagger.stats._params_nominal, eta_plot, tagger.stats._tagdata.decay_flav)
...
def confidence_bands_lineshape(tagger, eta_plot, dec_plot):
    ...
    J = tagger.func.gradient(tagger.stats._params_nominal, eta_plot, dec_plot).T
...
def print_fit_setup(ax, position, tagger, uformat="{:L}"):
    ...
    param = [f"${p} = {np.round(n, 4)} ± {np.round(s, 4)}$" for n, s, p in zip(noms, uncts, param)]
