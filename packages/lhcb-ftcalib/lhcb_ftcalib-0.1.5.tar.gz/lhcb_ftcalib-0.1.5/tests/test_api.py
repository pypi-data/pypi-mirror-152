...
def test_calibfunction_usage():
    ...
    for functype in ft.calibration_functions.CalibrationFunction.__subclasses__():
        ...
        for link in ft.link_functions.link_function.__subclasses__():
            for deg in range(0, 10):
                if functype is ft.PolynomialCalibration:
                    ...
                    assert len(func.param_names) == 2 * deg
                    assert len(func.param_names_delta) == 2 * deg
                    ...
                    result = func.eval_ignore_delta(params, eta)
                    assert id(result) != id(eta)
                    assert np.array_equal(eta, etaref)
                    assert np.array_equal(dec, decref)
                    ...
                    assert len(func.param_names) == 2 * (deg + 2)
                    assert len(func.param_names_delta) == 2 * (deg + 2)
                    assert func.link == link
                    params = (2 * func.npar) * [0]
