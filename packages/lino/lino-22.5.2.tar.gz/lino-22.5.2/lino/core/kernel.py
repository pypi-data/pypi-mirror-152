...
class Kernel(object):
    ...
    def code_mtime(self):
        ...
        if self._code_mtime is None:
            if self.site.is_demo_site:
                ...
                self._code_mtime = os.stat(import_module(
                    os.environ['DJANGO_SETTINGS_MODULE']).__file__).st_mtime
    ...
    def lino_version(self):
        ...
        This gathers all of the apps version numbers and add them up in order,
        to use it as modification stamp. Any updates on any of the dependent
        django apps will increase this collective version number and will be
        used as a stamp to propagate changes through the system.

        This is automatically activated when :attr:`Site.never_build_site_cache
        <lino.core.site.Site.never_build_site_cache>` is set to `True`,
        supposedly used in a production server.

        """
        if self._lino_version is None:
            mods = set([mod.split('.')[0] for mod in settings.INSTALLED_APPS])
            version = [0, 0, 0]
            for mod in mods:
                m = import_module(mod)
                if hasattr(m, '__version__'):
                    v = [int(num) for num in m.__version__.split('.')]
                    for i in range(len(v)):
                        version[i] += v[i]
                        if i == 2:
                            break
            rn = [int(num) for num in self.site.revision_number.split('.')]
            for i in range(rn):
                version[i] = version[i] + rn[i]
            version = [str(num) for num in version]
            self._lino_version = '.'.join(version)
    ...
    def make_cache_file(self, fn, write, force=False):
        ...
        #     return 0
        fn = join(settings.SITE.site_cache_root, fn)
