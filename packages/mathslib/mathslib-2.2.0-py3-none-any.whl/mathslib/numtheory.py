...
def FrobeniusNumber(*integers):
    '''
    Generates the `Frobenius Number <https://en.wikipedia.org/wiki/Coin_problem>`_ for a set A
    Given positive integers a1, a2, ..., an such that gcd(a1, a2, ..., an) = 1, the largest integer that cannot be expressed as an integer conical combination of these numbers, i.e., as a sum
    k1*a1 + k2*a2 + ··· + kn*an, is called the Frobenius number.
    
    The below algorithm is based off `Faster Algorithms for Frobenius Numbers <https://www.cis.upenn.edu/~cis511/Frobenius-numbers-Nijenhuis-Wagon.pdf>`_
    The following is an implementation of their Breadth-First Method
