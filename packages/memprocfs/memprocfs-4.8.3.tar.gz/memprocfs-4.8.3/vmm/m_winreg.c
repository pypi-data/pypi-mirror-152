...
NTSTATUS MWinReg_Read_KeyInfo(POB_REGISTRY_HIVE pHive, _In_ LPSTR uszKeyPath, _In_ BOOL fMeta, _Out_writes_to_(cb, *pcbRead) PBYTE pb, _In_ DWORD cb, _Out_ PDWORD pcbRead, _In_ QWORD cbOffset)
{
...
NTSTATUS MWinReg_Read_KeyValue(_In_ LPSTR uszPathFull, _Out_writes_to_(cb, *pcbRead) PBYTE pb, _In_ DWORD cb, _Out_ PDWORD pcbRead, _In_ QWORD cbOffset)
{
    NTSTATUS nt = VMMDLL_STATUS_FILE_INVALID;
    ...
    PBYTE pbData = NULL;
    LPSTR szMeta = NULL;
    if(!VmmWinReg_PathHiveGetByFullPath(uszPathFull, &pObHive, uszSubPath)) { goto finish; }
    cuszSubPath = (DWORD)strlen(uszSubPath);
    // key info file
    if(CharUtil_StrEndsWith(uszSubPath, "\\(_Key_)", TRUE)) {
        uszSubPath[cuszSubPath - 8] = 0;
        return MWinReg_Read_KeyInfo(pObHive, uszSubPath, FALSE, pb, cb, pcbRead, cbOffset);
    }
    if(CharUtil_StrEndsWith(uszSubPath, "\\(_Key_).txt", TRUE)) {
        uszSubPath[cuszSubPath - 12] = 0;
        return MWinReg_Read_KeyInfo(pObHive, uszSubPath, TRUE, pb, cb, pcbRead, cbOffset);
    }
    // raw registry value - i.e not metadata
    if((cuszSubPath < 5) || strcmp(uszSubPath + cuszSubPath - 4, ".txt")) {
        nt = VmmWinReg_ValueQuery1(pObHive, uszSubPath, NULL, NULL, pb, cb, pcbRead, cbOffset) ? VMMDLL_STATUS_SUCCESS : VMMDLL_STATUS_END_OF_FILE;
        goto finish;
    }
    // metadata file below:
    ...
    if(!VmmWinReg_ValueQuery1(pObHive, uszSubPath, &dwType, &raValue, pbData, 2 * KEY_META_BUFFER_SIZE, &cbData, 0)) {
        VmmWinReg_ValueQuery1(pObHive, uszSubPath, &dwType, &raValue, NULL, 0, &cbData, 0);
    }
...
NTSTATUS MWinReg_Read(_In_ PVMMDLL_PLUGIN_CONTEXT ctx, _Out_writes_to_(cb, *pcbRead) PBYTE pb, _In_ DWORD cb, _Out_ PDWORD pcbRead, _In_ QWORD cbOffset)
{
    ...
    LPSTR uszSubPath;
    *pcbRead = 0;
    uszSubPath = CharUtil_PathSplitFirst(ctx->uszPath, uszTopPath, sizeof(uszTopPath));
    if(!_stricmp(uszTopPath, "hive_files")) {
        pObHive = VmmWinReg_HiveGetByAddress(Util_GetNumericA(uszSubPath));
    ...
    if(!_stricmp(uszTopPath, "by-hive") || !_stricmp(uszTopPath, "HKLM") || !_stricmp(uszTopPath, "HKU")) {
        return MWinReg_Read_KeyValue(ctx->uszPath, pb, cb, pcbRead, cbOffset);
    }
...
VOID MWinReg_List_KeyAndValue(_Inout_ PHANDLE pFileList, _In_ POB_REGISTRY_HIVE pHive, _In_ LPSTR uszPath)
{
    ...
    if(uszPath[0] && !(pObKey = VmmWinReg_KeyGetByPath(pHive, uszPath))) { return; }
    // list key info
...
BOOL MWinReg_List(_In_ PVMMDLL_PLUGIN_CONTEXT ctx, _Inout_ PHANDLE pFileList)
{
    ...
    if(!_strnicmp(uszPath, "by-hive", 7)) {
        ...
        // list hive contents
        if(VmmWinReg_PathHiveGetByFullPath(uszPath, &pObHive, uszPathHive)) {
            MWinReg_List_KeyAndValue(pFileList, pObHive, uszPathHive);
