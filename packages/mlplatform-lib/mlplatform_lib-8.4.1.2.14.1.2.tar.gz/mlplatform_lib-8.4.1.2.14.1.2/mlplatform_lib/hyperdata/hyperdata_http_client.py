...
class HyperdataHttpClient:
        ...
        file_download = FileDownload(
            ...
            selector_property=SelectorProperty(
                column_select_options=[
                    ColumnOption(name=col_info.name, alias=col_info.name, type="VARCHAR")
    ...
    def download_unstructured(self, do_id:int):
        print("Start download_unstructured in hyperdata http client")
        return self.send_request(
            "download/ozone/unstructured",
            {"dataobjects": do_id},
            {"fileName":do_id,"regex":,"dateStart":,"dateEnd":},
            {},
            HyperdataRequestType.GET,
        )
