...
def _nearest_coeffs(ratio, mode=b'round_prefer_floor'):
    if type(ratio) == int or ratio.is_integer():
        return numpy.array([0, 1])
    if mode == b'round_prefer_floor':
        return numpy.array([ratio <= 0.5, ratio > 0.5])
    if mode == b'round_prefer_ceil':
        return numpy.array([ratio < 0.5, ratio >= 0.5])
    if mode == b'floor':
...
def _interpolate_1d_with_x(
        data, scale_factor, x, get_coeffs, roi=None,
        extrapolation_value=0.0, coordinate_transformation_mode=b'half_pixel',
    ...
    elif coordinate_transformation_mode == b'asymmetric':
        x_ori = x / scale_factor
    elif coordinate_transformation_mode == b'tf_crop_and_resize':
        if output_width == 1:
    ...
    elif coordinate_transformation_mode == b'pytorch_half_pixel':
        if output_width == 1:
            x_ori = -0.5
        else:
            x_ori = (x + 0.5) / scale_factor - 0.5
    elif coordinate_transformation_mode == b'half_pixel':
        x_ori = (x + 0.5) / scale_factor - 0.5
    else:
        raise ValueError('invalid coordinate_transformation_mode: %r.' %
...
def _interpolate_nd_with_x(data, n, scale_factors, x,
                           get_coeffs, roi=None, **kwargs):
    if n == 1:
        return _interpolate_1d_with_x(
            data, scale_factors[0], x[0], get_coeffs, roi=roi, **kwargs)
    return _interpolate_1d_with_x(
        [_interpolate_nd_with_x(
            data[i], n - 1, scale_factors[1:], x[1:], get_coeffs,
            roi=None if roi is None else numpy.concatenate(
                [roi[1:n], roi[n + 1:]]), **kwargs)
         for i in range(data.shape[0])],
        scale_factors[0], x[0], get_coeffs,
...
def _interpolate_nd(data, get_coeffs, output_size=None,
    ...
    for x in _get_all_coords(ret):
        ret[tuple(x)] = _interpolate_nd_with_x(
            data, len(data.shape), scale_factors, x, get_coeffs,
            roi=roi, **kwargs)
    return ret
...
class Resize(OpRun):
    ...
    def __init__(self, onnx_node, desc=None,
        ...
        OpRun.__init__(self, onnx_node, desc=desc,
                       expected_attributes=Resize.atts,
                       **options)
