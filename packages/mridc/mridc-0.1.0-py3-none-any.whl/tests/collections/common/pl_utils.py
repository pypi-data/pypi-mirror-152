        ...
        if loss_metric.dist_sync_on_step:
            if rank == 0:
                ...
                if sk_batch_result.isnan():
                    if check_dist_sync_on_step and not batch_result.isnan():
                        raise AssertionError
                elif not np.allclose(batch_result.numpy(), sk_batch_result, atol=atol) and check_dist_sync_on_step:
                    raise AssertionError(
            ...
            if sk_batch_result.isnan():
                if check_batch and not batch_result.isnan():
                    raise AssertionError
            elif not np.allclose(batch_result.numpy(), sk_batch_result, atol=atol) and check_batch:
                raise AssertionError(
                    f"batch_result = {batch_result.numpy()}, sk_batch_result = {sk_batch_result}, i = {i}"
                )
    # check on all batches on all ranks
