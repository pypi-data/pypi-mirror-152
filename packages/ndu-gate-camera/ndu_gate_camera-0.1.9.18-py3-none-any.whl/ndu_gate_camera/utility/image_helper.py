...
def select_areas(frame, window_name, color=(0, 0, 255), opacity=0.3, thickness=4, max_count=None, next_area_key="n",
                 finish_key="s", return_tuples=True, max_point_count=None):
    try:
        areas = []
        area = []
...
def normalize_multiple_coordinates(image, coords):
    h, w = image.shape[:2]
    pnts = []
    for coord in coords:
        for x, y in coord:
...
def denormalize_coordinates_intersector(image, coords):
    h, w = image.shape[:2]
    pnts = []
    for x, y in coords:
        pnts.append([int(x * w), int(y * h)])
...
def normalize_coordinates_intersector(image, coords):
    h, w = image.shape[:2]
    pnts = []
    for x, y in coords:
        pnts.append([x / w, y / h])
...
def set_frames_path(_path):
    ...
    # create video using old, remained frames
    video_link = create_video()
    # size check
    if get_size(path_file) > 5000000000:  # 5000000000
        dir_list = next(os.walk(path_file))[1]
        dir_list.sort()
        shutil.rmtree(path_file + path.sep + dir_list[0])
    return video_link
...
def get_frames_number():
    from os import walk
    global path_file
    filenames = next(walk(path_file), (None, None, []))[2]  # [] if no file
    filenames = [f for f in filenames if f.endswith(frame_extension)]
...
def create_video():
    ...
    from os import walk
    global path_file
    dates = get_immediate_subdirectories(path_file)
    video_link = ''
    for date_dir in dates:
        cameras = get_immediate_subdirectories(path_file + path.sep + date_dir)
        for camera in cameras:
            frames = []
            filenames = next(walk(path_file + path.sep + date_dir + path.sep + camera), (None, None, []))[2]  # [] if no file
            # remove if file does not end with frame extension
            filenames = [f for f in filenames if f.endswith(frame_extension)]
            filenames.sort()
            for filename in filenames:
                if filename.endswith(frame_extension):
                    frames.append(cv2.imread(path_file + path.sep + date_dir + path.sep + camera + path.sep + filename))
            if len(frames) > 1:
                height, width, _ = frames[0].shape
                first_frame_name = filenames[0][:-4].replace(" ", "_").replace(":", ".")
                last_frame_name = filenames[-1][:-4].replace(" ", "_").replace(":", ".")
                device = first_frame_name.split("_")[0]
                date = first_frame_name.split("_")[1]

                if not os.path.exists(path_file + path.sep + date):
                    os.makedirs(path_file + path.sep + date)

                if not os.path.exists(path_file + path.sep + date + path.sep + device):
                    os.makedirs(path_file + path.sep + date + path.sep + device)

                first_name = first_frame_name.split("_")[1] + "_" + first_frame_name.split("_")[2]
                last_name = last_frame_name.split("_")[1] + "_" + last_frame_name.split("_")[2]

                out = cv2.VideoWriter(
                    path_file + path.sep + date + path.sep + device + path.sep +
                    first_name + '--' +
                    last_name + ".mp4",
                ...
                out.release()
                from ndu_gate_camera.utility import ndu_utility
                video_link = (ndu_utility.NDUUtility.host + path_file + path.sep + date + path.sep + device + path.sep +
                    first_name + '--' +
                    last_name + ".mp4")
            ...
            for filename in filenames:
                if filename.endswith(frame_extension):
                    try:
                        os.remove(path_file + path.sep + date_dir + path.sep + camera + path.sep + filename)
                    except:
                        pass
            if video_link != '':
                return video_link
