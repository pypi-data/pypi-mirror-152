...
from ndu_gate_camera.utility.download_zipped_helper import download_runner_data
...
class NDUCameraService(Thread):
    def __init__(self, instance=None, config_dir="", handler=None, is_main_thread=True, extension_folder=None, config_file="", timestamp="", updater=None):
        ...
        self._ndu_config_file = config_file
    ...
    def _load_download_urls(self, ):
        load_download_urls = path.join(self._ndu_gate_config_dir, 'data_download_urls.yaml')
        print("_load_download_urls config file : {}".format(load_download_urls))
        try:
            with open(load_download_urls, encoding="utf-8") as general_config:
                self.download_urls = safe_load(general_config)

        except:
            print('TODO - print exception details')
        ...
        if self.RUNNERS:
            for runner in self.RUNNERS:
                ...
                try:
                    ...
                    runner_class_info = NDUUtility.check_and_import(runner_type, class_name,
                                                                    package_uuids=runner.get("uuids", None),
                                                                    extension_folder=self.__extension_folder)

                    if runner_class_info is None or runner_class_info[0] is None:
                        log.warning("class name implementation not found for %s - %s", runner_type, class_name)
                        continue

                    runner_class = runner_class_info[0]
                    ...
                    rt = runners_configs_temp[runner_unique_key] = {
                        ...
                        "runner_load_path": runner_class_info[1]
                    }
    ...
    def _connect_with_runners(self):
        ...
        for runner_config in self.runners_configs:
            ...
            try:
                ...
                # data klasörünün hangi runner klasörüne indirileceği problemi var, şimdilik projedeki runners klasörüne indirsin
                download_runner_data(runner_type, runner_config["runner_load_path"], self.download_urls)
    ...
    def _run(self):
        ...
        try:
            ...
            for _frame_index, frame in self.video_source.get_frames():
                if self.__result_handler.get_restart_required() or (self.updater is not None and self.updater.exit_required):
                    self._exit_requested = True
                    '''
                    try:
                ...
                self.__result_handler.set_frame(device, camera_capture_base64)
                if i % self.__frame_send_interval == 0:
                    try:
                        ...
                        #print(camera_capture_base64)

                        if self.frame_sent:
                            if len(camera_capture_base64) < 45000:
                ...
                if constants.MOBILE_RUNNER in self.available_runners:
                    frame = image_helper.resize(frame, 320, 320)
                else:
                    ...
                    for runner_unique_key in self.available_runners:
                        try:
                            ...
                            if self.__preview_show:
                                ...
                                #self.__result_handler.save_result([{'data': {runner_conf["type"]: (1.0 / max(elapsed, 0.001))}}], device=device, runner_name=runner_conf["type"])
                ...
                if self.__preview_show:
                    if self._skip > 0:
                        ...
                        # self.__result_handler.save_result(
                        #     [{'data': {'fps':  (1.0 / max(total_elapsed_time, 0.001))}}], device=device,
                        #     runner_name=runner_conf["type"])
