...
from ndu_gate_camera.updater import Updater
...
def _get_config():
    ...
    if ndu_gate_config_file and path.sep not in ndu_gate_config_file:
        ndu_gate_config_file = path.join(path.dirname(path.abspath(__file__)) + '/config/'.replace('/', path.sep),
...
def main(ndu_gate_config_file):
    ...
    try:
        ...
        timestamp = os.stat(ndu_gate_config_file).st_mtime
        ...
        plate_service_url = ndu_gate_config.get("plate_service_url", None)

        result_hand_conf = ndu_gate_config.get("result_handler", None)
        use_platform = ndu_gate_config.get("platform", False)
        ...
        if str(res_handler) == str("SOCKET"):
            ...
            result_handler = ResultHandlerMqtt(result_hand_conf.get("access_token"), result_hand_conf.get("mqtt"), ndu_gate_config_file, plate_service_url, use_platform)
        ...
        if len(instances) > 1:
            ...
            for instance in instances:
                ...
                camera_service = NDUCameraService(instance=instance, config_dir=ndu_gate_config_dir,
                                                  ...
                                                  extension_folder=extension_folder, config_file=ndu_gate_config_file, timestamp=timestamp, updater=updater)
            ...
            camera_service = NDUCameraService(instance=instances[0], config_dir=ndu_gate_config_dir,
                                              handler=result_handler, is_main_thread=True,
                                              extension_folder=extension_folder, config_file=ndu_gate_config_file, timestamp=timestamp, updater=updater)
            camera_service.run()
        else:
        ...
        import subprocess
        if updater.exit_required:
            os.system("TASKKILL /F /IM ndugateapp.exe")
            file_dir = os.path.dirname(os.path.realpath(__file__))
            subprocess.Popen([os.path.join(file_dir, 'ndu_gate_update.exe'), '/SILENT'])
        if result_handler.get_restart_required():
            result_handler.dispose()
            main(ndu_gate_config_file)
