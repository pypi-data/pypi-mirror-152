...
def main():
    ...
    print(tleap,sander,sander_MPI,cpptraj)
    ...
    if args.env is not None:
        ...
        condor_combine = open("run_combine.sub","w")  ###  scripts dir will need to read from package install
        condor_combine.write("executable = run_refine.sh\nrequest_memory = 4G\nrequest_cpus = 1\nrequest_gpus = 0\noutput = logs/$(JobName).stdout\nerror = logs/$(JobName).stderr\nlog = logs/run.log\n")
        ...
        condor_ansurr = open("run_ansurr.sub","w")  ###  scripts dir will need to read from package install
        condor_ansurr.write("executable = run_ansurr.sh\nrequest_memory = 4G\nrequest_cpus = 1\nrequest_gpus = 0\noutput = logs/$(JobName).stdout\nerror = logs/$(JobName).stderr\nlog = logs/run.log\n")
        ...
        condor_bme = open("run_bme.sub","w")  ###  scripts dir will need to read from package install
        condor_bme.write("executable = run_bme.sh\nrequest_memory = 4G\nrequest_cpus = 1\nrequest_gpus = 0\noutput = logs/$(JobName).stdout\nerror = logs/$(JobName).stderr\nlog = logs/run.log\n")
        ...
        bash_refine.write("python3 "+str(refine_script)+" $1 $2 $3 $4 $5 $6 $7 $8"+'\n')
        bash_refine.close()
        os.chmod("run_refine.sh" , 0o777)

        bash_ansurr = open("run_ansurr.sh","w")
        ...
        bash_ansurr.close()
        os.chmod("run_ansurr.sh" , 0o777)

        bash_bme = open("run_bme.sh","w")
        ...
        bash_bme.close()
        os.chmod("run_bme.sh" , 0o777)


        dag = open(job_id+".dag","w")
        dag.write("JOB A run_refine.sub\n")
        dag.write("JOB B run_combine.sub\n")
        dag.write("JOB C run_ansurr.sub\n")
        dag.write("JOB D run_bme.sub\n")
