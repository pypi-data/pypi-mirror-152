...
def refine(home_dir, job_id, pdb, cpu, tleap, sander, sander_MPI, cpptraj, quiet=False):
    ...
    if int(cpu)==1:  
        check_quiet_print(quiet,"refining models with 1 cpu using sander")
        ...
        check_quiet_print(quiet,"refining models with "+str(cpu)+" cpus using sander.MPI")
...
def combine(home_dir, job_id, pdb_id,quiet=False):
    ...
    if refined_pdbs > 0:
        check_quiet_print(quiet,"sucessfully refined "+str(refined_pdbs)+" models")
    else:
        check_quiet_print(quiet,"ERROR failed to refine any models, quitting")
        sys.exit(0)
