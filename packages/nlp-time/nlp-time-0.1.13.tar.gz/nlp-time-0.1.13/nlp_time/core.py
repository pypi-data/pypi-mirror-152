...
def get_time(text: str, tend_future=True, time_base=None):
    ...
    try:
        ...
        if not time_obj:
            return "", time_tuple
        time_obj = time_obj[0]
        if time_obj.get("type", "") not in ("time_span", "time_point"):  # not time_delta and time_period
            return "", time_tuple
        ...
        if time_dict.get("type", "") == "time_point":
            if re.findall(RE_NOT_THIS_WEEK, text):
                if tend_future:
                    if t1 and t1 < now:
...
def get_text(dt: Union[datetime, str, int], time_base=None):
    if not time_base:
        time_base = datetime.now()
    elif isinstance(time_base, (int, float)):
        time_base = datetime.fromtimestamp(time_base)
    if isinstance(dt, str):
        dt = datetime.strptime(dt, TIME_FORMAT)
    elif isinstance(dt, int):
        dt = datetime.fromtimestamp(dt)
    duration = time_base - dt
    today_zero = time_base - relativedelta(hour=0, minute=0, second=0, microsecond=0)
    ...
    if not duration.days:
        ...
        if -2 <= days_span_today_zero <= 2:
            reply.append(around_today(days_span_today_zero))
        elif dt.year != time_base.year:
            month_year = time_base.year - dt.year
            if month_year:
                _m = {1: "去", -1: "明"}
                reply.append(f"{_m.get(month_year, dt.year)}年")
            reply.append(f"{dt.month}月")
            reply.append(f"{dt.day}号")
        elif dt.month != time_base.month:
            month_span = time_base.month - dt.month
            if month_span:
                _m = {1: "上个", -1: "下个"}
                reply.append(f"{_m.get(month_span, dt.month)}月")
            reply.append(f"{dt.day}号")
        else:
            week_span = dt.day - (time_base.day - time_base.weekday())
            if -7 <= week_span < 0:
...
RE_NOT_THIS_WEEK = rf"(?<!这个)(?<!这|本){RE_WEEK}"
