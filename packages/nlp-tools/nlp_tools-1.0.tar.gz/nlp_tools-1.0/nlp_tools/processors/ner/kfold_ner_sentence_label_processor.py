...
class KfoldNerSentenceLabelProcessor(ABCLabelProcessor):
    """
    Generic processors for the sequence samples.
    """

    def to_dict(self) -> Dict[str, Any]:
        data = super(KfoldNerSentenceLabelProcessor, self).to_dict()
        data['config'].update({
    ...
    def __init__(self,
        ...
        Args:
            ...
            **kwargs:
        """
        super(KfoldNerSentenceLabelProcessor, self).__init__(**kwargs)
    ...
    def build_vocab_generator(self,generators) -> None:
        if not self.vocab2idx:
            ...
            for label in list(set(tag_set)):
                ...
                elif self.tag_mode == "BIES":
                    vocab2idx["B-" + label] = len(vocab2idx)
                    vocab2idx["I-" + label] = len(vocab2idx)
                    vocab2idx["E-" + label] = len(vocab2idx)
                    vocab2idx["S-" + label] = len(vocab2idx)
            ...
            for sub_label in sample_labels:
                ...
                if self.tag_mode == "BI":
                    ...
                    if start_position == end_position:
                        labels_numpy[index, start_position] = self.vocab2idx["S-" + label_type]
                    else:
                        labels_numpy[index, start_position] = self.vocab2idx["B-" + label_type]
                        for sub_index in range(start_position + 1, end_position):
                            labels_numpy[index, sub_index] = self.vocab2idx["I-" + label_type]
                        labels_numpy[index, end_position] = self.vocab2idx["E-" + label_type]
    ...
    def transform_kfold(self,samples,seq_length=None):
        if not seq_length:
            seq_length = self.max_sentence_length
        ...
        labels_numpy = np.zeros((len(samples), seq_length,len(self.vocab2idx)))
        ...
        for index, sample_labels in enumerate(samples):
            for labels in sample_labels:
                ...
                for sub_label in labels:
                    ...
                    if self.tag_mode == "BI":
                        used_index.add(start_position)
                        labels_numpy[index, start_position, self.vocab2idx["B-" + label_type]] += 1
                        for sub_index in range(start_position +1,end_position+1):
                            used_index.add(sub_index)
                            labels_numpy[index, sub_index,self.vocab2idx["I-" + label_type]] += 1
                    else:
                        used_index.add(start_position)
                        if start_position == end_position:
                            labels_numpy[index, start_position,self.vocab2idx["S-" + label_type]] += 1
                        else:
                            labels_numpy[index, start_position , self.vocab2idx["B-" + label_type]] += 1
                            for sub_index in range(start_position + 1, end_position):
                                used_index.add(sub_index)
                                labels_numpy[index, sub_index,self.vocab2idx["I-" + label_type]] += 1
                            labels_numpy[index, end_position,self.vocab2idx["E-" + label_type]] += 1
                            used_index.add(end_position)
                for idx in range(seq_length):
                    if idx not in used_index:
                        labels_numpy[index, idx, self.vocab2idx["O"]] += 1

        def softmax(x, axis=-1):
            #x = x - x.max(axis=axis, keepdims=True)
            y = np.exp(x)
            masking = np.int64(np.array(x)>0)
            y = y* masking
            return y / y.sum(axis=axis, keepdims=True)

        labels_numpy = softmax(labels_numpy)
        return labels_numpy
