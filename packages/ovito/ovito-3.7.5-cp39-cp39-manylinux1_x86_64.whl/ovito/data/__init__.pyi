...
__all__ = ['DataCollection', 'DataObject', 'TriangleMesh', 'SimulationCell', 'Property', 'PropertyContainer', 'DataTable', 'ElementType', 'SurfaceMesh', 'VoxelGrid', 'ParticleType', 'BondType', 'BondsEnumerator', 'CutoffNeighborFinder', 'NearestNeighborFinder', 'PTMNeighborFinder', 'Particles', 'Bonds', 'TrajectoryLines', 'DislocationNetwork', 'DislocationSegment']
...
Typically, surface meshes are produced within OVITO by modifiers such as the :py:class:`ConstructSurfaceModifier`, 
:py:class:`CreateIsosurfaceModifier` or :py:class:`CoordinationPolyhedraModifier`. 
Please see the user manual page on surface meshes for a more in-depth description
of this geometric data structure.

Periodic domains

Surface meshes may be embedded in periodic domains, i.e. in simulation cells with periodic boundary conditions applied. 
...
OVITO takes care of computing the intersections of the periodic surface with the box boundaries and automatically produces a non-periodic 
version of the mesh when it comes to visualization. You can also request a non-periodic version of the mesh, which is clipped at the 
periodic box boundaries, by calling the :py:meth:`to_triangle_mesh` method.
...
may, in some situations, not be identical to the global simulation :py:attr:`cell` set for the :py:class:`DataCollection`.
...
A set of *cutting planes* can be assigned to a :py:class:`SurfaceMesh` to clip away parts of the mesh for visualization purposes.
...
methods. Note that the cuts are non-destructive and get performed only on the transient, non-periodic version of the mesh generated during image rendering or when exporting 
the mesh to a file. The original surface mesh is not affected.
The :py:class:`SliceModifier`, when applied to a :py:class:`SurfaceMesh`, performs the slice by simply adding a corresponding cutting plane 
...
surface mesh."""
    ...
    def vertices(self) -> PropertyContainer:
        """The :py:class:`PropertyContainer` storing the per-vertex properties of the mesh, including the vertex coordinates."""
        ...
    ...
    def get_cutting_planes(self) -> numpy.typing.NDArray:
...
Note that the returned Numpy array is a copy of the internal data stored by the :py:class:`SurfaceMesh`."""
    ...
    def get_faces(self) -> numpy.typing.NDArray:
        """Returns a *M* x 3 array with the vertex indices of the *M* triangles in the mesh. Note that the returned Numpy array is a copy of the internal data stored by the :py:class:`SurfaceMesh`. Also keep in mind that triangle faces can cross the domain boundaries if the periodic boundary conditions are used."""
        ...

    def get_vertices(self) -> numpy.typing.NDArray:
        """Returns a *N* x 3 array with the xyz coordinates of the *N* vertices in the mesh. Note that the returned Numpy array is a copy of the internal data stored by the :py:class:`SurfaceMesh`."""
        ...
...
:return: The numeric ID of the spatial region containing *pos*; or ``None`` if *pos* is exactly on the dividing surface between two regions"""
        ...
...
- `trimesh`: A :py:class:`TriangleMesh` representing the surface geometry after clipping it at the periodic boundaries of the   :py:attr:`.domain` and any attached cutting planes (see :py:meth:`get_cutting_planes`).
...
A triangle mesh is a more low-level data structure, which can be sent directly to a graphics card for rendering. A surface mesh, in contrast,
is a more high-level data structure, which is automatically converted to a triangle mesh for visual rendering.
