        ...
        for entry in self.entry_list:
            for output_key in entry['output_keys']:
                ...
                if output_value == None:
                    # Cost Disabled resources may not have their registered
                    # StackOutputs
                    if entry['stack'].resource and hasattr(entry['stack'].resource, 'cost_disabled'):
                        continue
                    else:
                        raise PacoException(
...
""".format(param_entry.key, self.resource.paco_ref_parts, param_entry.stack.resource.paco_ref_parts, param_entry.stack.get_name())
        ...
        # Hooks
        new_hooks_cache_id = self.hooks.gen_cache_id()
        new_tags_cache_id = self.tags.gen_cache_id()

        new_cache_id += new_hooks_cache_id
        new_cache_id += new_tags_cache_id
...
""".format(param_entry.key, self.resource.paco_ref_parts, param_entry.stack.resource.paco_ref_parts, param_entry.stack.get_name())
    ...
    def get_outputs_value(self, key):
        ...
        if self.resource and hasattr(self.resource, 'cost_disabled'):
            return None
...
A Stack can cache it's templates to the filesystem or check them against AWS and get their status.
    ...
    def provision(self):
        ...
        # Universal Paco-Reference
        # KMS is held back because it does not allow updates after creation
        # Need a way to recreate the KMS, but its not needed for now.
        if self.template.resource != None and self.get_name().split('-')[-1] != 'KMS':
            self.tags.add_tag('Paco-Reference', self.template.resource.paco_ref_parts)
