...
from paco.models.applications import Application, PinpointApplication, ResourceGroups, ResourceGroup, \
    ...
    ECSTaskDefinitionSecret, ECSContainerDependency, \
...
def iterate_nested_dict(nested_dict, parent_obj, user_callback, callback_data, paco_ref_list=[]):
    orig_paco_ref_list = paco_ref_list
    for key, value in nested_dict.items():
        paco_ref_list = deepcopy(orig_paco_ref_list)
        if isinstance(value, dict):
            paco_ref_list.append(key)
            nested_dict[key] = iterate_nested_dict(nested_dict[key], nested_dict, user_callback, callback_data, paco_ref_list)
...
Caveat: You can not have an environment named 'applications'.
    ...
    def process_aliases(self, netenv_name, config):
        ...
        return iterate_nested_dict(config, None, self.process_aliases_user_callback, callback_data=netenv_name)
