...
def file_to_s3(s3_client_, file_path, s3_path):
    ...
    s3_client_.upload_file(file_path,
                           '{upload_path}/{filename}'.format(upload_path=s3_path, filename=os.path.basename(file_path)))
...
class PGS3CH:
    ...
    def extract_to_s3(self):
        ...
        for _id in range(0, end):
            ...
            logger.info("Copying to S3: %s", self.s3_config["S3_ENDPOINT_URL"])
