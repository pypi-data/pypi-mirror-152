import os, string, random, json, sys, subprocess, fcntl, time, traceback, socket, urllib.request, tarfile, glob
...
def _download_server(config):
    if os.path.exists(config.exp_postgres_bin):
        return
    bin_dir = config.exp_bin_directory
    ...
    if not os.path.exists(config.exp_postgres_bin):
        print(f"Download and extract to {bin_dir} failed.", file=sys.stderr)
        sys.exit(1)
...
def _auto_start(config):
    ...
    if daemon_fd is None:
        ...
        if not os.path.exists(config.directory+"/pgdata/postgresql.conf"):
            ...
            ], env=dict(os.environ, LD_LIBRARY_PATH=config.exp_system_lib_directory), check=True, stderr=log_fd, stdout=log_fd)
...
class Config:
    ...
    def get_system_lib_directory(self):
        return f"{self.bin_directory}/system-lib"
    ...
    def get_ld_linux(config):
        return glob.glob(config.exp_system_lib_directory+"/ld-linux-*")[0]
...
def _run_server(daemon_fd, log_fd, config):
    ...
    try:
        proc = subprocess.Popen([
            ...
            "-c", f"dynamic_library_path={config.exp_bin_directory}/lib",
            "-D", config.directory+"/pgdata",
            "-p", str(config.port),
            f"--unix_socket_directories={config.directory}"
        ], env=dict(os.environ, LD_LIBRARY_PATH=config.exp_system_lib_directory), stderr=log_fd, stdout=log_fd)
