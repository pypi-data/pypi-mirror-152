...
def daily_factor_on300500(fac,hs300=False,zz500=False,zz1000=False,gz2000=False,other=False):
    ...
    if fac.shape[0]/last.shape[0]>2:
        if hs300:
            ...
            df=pd.read_feather('/Users/chenzongwei/pythoncode/数据库/中证1000成分股.feather').set_index('index').replace(0,np.nan)
            df=df*fac
            df=df.dropna(how='all')
        elif gz2000:
            df=pd.read_feather('/Users/chenzongwei/pythoncode/数据库/国证2000成分股.feather').set_index('index').replace(0,np.nan)
            df=df*fac
            df=df.dropna(how='all')
        elif other:
            tr=read_daily(tr=1).fillna(0).replace(0,1)
            tr=np.sign(tr)
            df1=(tr*pd.read_feather('/Users/chenzongwei/pythoncode/数据库/沪深300成分股.feather').set_index('date')).fillna(0)
            df2=(tr*pd.read_feather('/Users/chenzongwei/pythoncode/数据库/中证500成分股.feather').set_index('date')).fillna(0)
            df3=(tr*pd.read_feather('/Users/chenzongwei/pythoncode/数据库/中证1000成分股.feather').set_index('index')).fillna(0)
        ...
        if hs300:
            ...
            df=pd.read_feather('/Users/chenzongwei/pythoncode/数据库/中证1000成分股.feather').set_index('index').replace(0,np.nan).resample('M').last()
            df=df*fac
            df=df.dropna(how='all')
        elif gz2000:
            df=pd.read_feather('/Users/chenzongwei/pythoncode/数据库/国证2000成分股.feather').set_index('index').replace(0,np.nan).resample('M').last()
            df=df*fac
            df=df.dropna(how='all')
        elif other:
            tr=read_daily(tr=1).fillna(0).replace(0,1).resample('M').last()
            tr=np.sign(tr)
            df1=(tr*pd.read_feather('/Users/chenzongwei/pythoncode/数据库/沪深300成分股.feather').set_index('date').resample('M').last()).fillna(0)
            df2=(tr*pd.read_feather('/Users/chenzongwei/pythoncode/数据库/中证500成分股.feather').set_index('date').resample('M').last()).fillna(0)
            df3=(tr*pd.read_feather('/Users/chenzongwei/pythoncode/数据库/中证1000成分股.feather').set_index('index').resample('M').last()).fillna(0)
