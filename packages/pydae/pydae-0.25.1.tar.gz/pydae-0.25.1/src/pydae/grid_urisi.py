...
import numpy as np
import sympy as sym
from pydae.pydgrid import grid
import json
...
def unb_ri_si(data):
    ...
    if 'vscs' in pydgrid_obj.data:
        for vsc in pydgrid_obj.data['vscs']:
            ...
            if vsc['type'] == 'ac3ph4wgfpi': # AC 3 phase 4 wire open loop PI-VSG grid former
                ac3ph4wgfpi(pydgrid_obj,vsc)
                pydgrid_obj.omega_coi_required = True
            if vsc['type'] == 'ac3ph4wgfpi2': # AC 3 phase 4 wire open loop PI-VSG grid former
                ac3ph4wgfpi2(pydgrid_obj,vsc)
                pydgrid_obj.omega_coi_required = True
            if vsc['type'] == 'ac3ph4wgfpidq': # AC 3 phase 4 wire open loop PI-VSG grid former
                ac3ph4wgfpidq(pydgrid_obj,vsc)
                pydgrid_obj.omega_coi_required = True
...
def ac3ph4wgfpi(grid,vsc_data):
    ...
    eq_i_sc_cplx = e_cm_cplx - i_sc*Z_sc - v_scn - v_mn
    eq_v_nm_cplx = 0*e_om_cplx - i_sn*Z_sn - v_mn 
    eq_i_sn_cplx = i_sa + i_sb + i_sc + i_sn + v_o/Z_ng
    #eq_i_sn_cplx = e_ng_cplx - i_sn*Z_sn - v_ng
    #eq_i_ng_cplx = i_ng + i_sa + i_sb + i_sc + i_sn
    #eq_e_ng_cplx  = -e_ng_cplx  + i_ng*Z_ng
    ...
    f_list += [dphi,dxi_p,dp_ef,dp_cf]
    x_list += [ phi, xi_p, p_ef, p_cf]
...
def ac3ph4wgfpi2(grid,vsc_data):
    ...
    # inputs
    e_ao_m,e_bo_m,e_co_m,e_no_m = sym.symbols(f'e_{name}_ao_m,e_{name}_bo_m,e_{name}_co_m,e_{name}_no_m', real=True)
    omega_ref,p_c = sym.symbols(f'omega_{name}_ref,p_c_{name}', real=True)
    phi_a = sym.Symbol(f'phi_a_{name}', real=True)
    phi_b = sym.Symbol(f'phi_b_{name}', real=True)
    phi_c = sym.Symbol(f'phi_c_{name}', real=True)
    ...
    e_ao_r = e_ao_m*sym.cos(phi_a + phi) 
    e_ao_i = e_ao_m*sym.sin(phi_a + phi) 
    e_bo_r = e_bo_m*sym.cos(phi_b + phi-2/3*np.pi) 
    e_bo_i = e_bo_m*sym.sin(phi_b + phi-2/3*np.pi) 
    e_co_r = e_co_m*sym.cos(phi_c + phi-4/3*np.pi) 
    e_co_i = e_co_m*sym.sin(phi_c + phi-4/3*np.pi) 
...
def ac3ph4wgfpidq(grid,vsc_data):
    ...
    f_list += [dphi,dxi_p,dp_ef,dp_cf,domega_f]
    x_list += [ phi, xi_p, p_ef, p_cf, omega_f]
