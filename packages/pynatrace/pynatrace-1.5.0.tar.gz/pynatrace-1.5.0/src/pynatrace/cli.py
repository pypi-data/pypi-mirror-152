...
def main():
    """
    pynatrace is a cli for sending logs to Dynatrace\n
...
@main.command('create-token', short_help='Create a new token with read/ingest metrics scopes')
def create_token():
    """ Create a new token """
    dt_api_key = os.getenv('DT_API_KEY')
    dt_server_uri = os.getenv('DT_SERVER_URI')
...
@main.command('get-alerts', short_help='Get the a list of alerting profiles')
...
@main.command('get-notifications', short_help='Get the a list of configured notifications')
@click.option("--details", is_flag=True)
...
@click.option("--severity", default='INFO', type=click.Choice(['INFO','WARN','ERROR'], case_sensitive=False))
