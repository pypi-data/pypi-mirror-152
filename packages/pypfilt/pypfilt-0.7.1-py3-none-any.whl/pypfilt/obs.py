...
class Obs(abc.ABC):
    ...
    def log_llhd(self, ctx, snapshot, obs, ixs=None):
        ...
        :param ixs: Sampling indices for the shape parameters (optional).
        """
        pass

    @abc.abstractmethod
    def expect(self, ctx, snapshot, ixs=None):
        """
    ...
    @abc.abstractmethod
    def quantiles(self, ctx, snapshot, probs, ixs=None):
        r"""
    ...
    @abc.abstractmethod
    def llhd_in(self, ctx, snapshot, y0, y1, ixs=None):
        """
...
class Univariate(Obs):
    ...
    Each observation comprises a time (``'date'``) and a floating-point value
    (``'value'``).
    ...
    .. note:: The observation unit is stored in the ``unit`` attribute (see
       the example below).
    ...
    def log_llhd(self, ctx, snapshot, obs, ixs=None):
        dist = self.distribution(ctx, snapshot, ixs)
    ...
    def quantiles(self, ctx, snapshot, probs, ixs=None):
        ...
        def cdf(y):
            ...
            return np.dot(snapshot.weights, cdf_values)
    ...
    def llhd_in(self, ctx, snapshot, y0, y1, ixs):
        dist = self.distribution(ctx, snapshot, ixs)
        # NOTE: we integrate discrete variables from `y0` to `y1 - 1`.
        if _is_discrete(dist):
            y0 = y0 - 1
            y1 = y1 - 1
        return np.dot(snapshot.weights, dist.cdf(y1) - dist.cdf(y0))
    ...
    def row_into_obs(self, row):
        """
        Return an observation with fields ``'date'``, ``'value'``, and
        ``'unit'``.
        """
        return {
            'date': row['date'],
    ...
    def simulated_field_types(self, ctx):
        ...
        """
        cols = [io.time_field('date'), ('value', np.float_)]
        return cols
    ...
    def simulated_obs(self, ctx, snapshot, rng, ixs=None):
        ...
        """
        values = self.simulate(ctx, snapshot, rng, ixs)
        return [{'date': snapshot.time, 'value': value} for value in values]
