...
class PytestHooks:
    ...
    def pytest_runtest_makereport(self, item: Item, call: CallInfo) -> Generator:
        ...
        report.test_rail_case_ids = list(itertools.chain(*[mark.args for mark in item.iter_markers("test_rail_case_id")]))
        report.xray_case_ids = list(itertools.chain(*[mark.args for mark in item.iter_markers("xray_test_key")]))
        report.zephyr_case_ids = list(itertools.chain(*[mark.args for mark in item.iter_markers("zephyr_test_case_key")]))
