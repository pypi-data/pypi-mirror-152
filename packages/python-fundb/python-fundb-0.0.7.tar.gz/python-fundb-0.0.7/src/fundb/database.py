...
def _parse_smart_filtering(filters: dict, indexed_columns:list=[]) -> dict:
    ...
    for k, v in filters.items():
        if query.FILTER_OPERATOR in k:
            f, op = k.split(query.FILTER_OPERATOR)
            if f in indexed_columns and op in query.SQL_OPERATORS:
                sql_filters.append((f, query.SQL_OPERATORS[op], v))
                continue
        else:
            if k in indexed_columns:
                sql_filters.append((k, query.SQL_OPERATORS["eq"], v))
                continue
        json_filters[k] = v
        ...
        if smart_filters["JSON_FILTERS"]:
            ...
            while True:
                ...
                if chunked:
                    ...
                    for r in query.execute(rows, smart_filters["JSON_FILTERS"]):
                        data.append(r)
                else:
                    break
            if data:
