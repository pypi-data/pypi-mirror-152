...
def parse_row_to_dict(row:dict) -> dict:
    ...
    """
    row = row.copy()
    _json = json_loads(row.pop("_json")) if "_json" in row else {}
    return {
        **_json,
...
class Database(object):
    ...
    def __init__(self, dsn:str=None, connector:adapters.Adapter = None):
        """
        Connect database 
        ...
        """
        if not dsn and not connector:
            connector = adapters.SQLiteAdapter(file=":memory:")
        elif dsn:
            _ = urlparse(dsn)
            # scheme, username, password, hostname, port, path.strip("/")
            if _.scheme == "sqlite":
                connector = adapters.SQLiteAdapter(dsn)
            elif _.scheme in ["mysql", "mariadb"]:
                connector = adapters.MySQLAdapter(dsn)
...
class Collection(object):
    ...
    def insert(self, doc: dict, _as_document:bool=True) -> Document:
        ...
        xparams = [_id, json_dumps(doc), ts, ts]
    ...
    def update(self, _id: str, doc: dict = {}, replace: bool = False, _as_document=True) -> Document:
        ...
        if rdoc:
            _doc = doc if replace else lib.dict_merge(json_loads(rdoc["_json"]), doc)
            ts = lib.get_timestamp()
            
            xcolumns = ["_json", "_modified_at"]
            xparams = [json_dumps(_doc), ts]
