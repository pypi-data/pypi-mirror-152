...
class Database(object):
    ...
    def __init__(self, dsn:str=None, adapter:adapters.Adapter = None):
        """
        Connect database 
        ...
        """
        if not dsn and not adapter:
            adapter = adapters.SQLiteAdapter(file=":memory:")
        elif dsn:
            _ = urlparse(dsn)
            # scheme, username, password, hostname, port, path.strip("/")
            if _.scheme == "sqlite":
                adapter = adapters.SQLiteAdapter(dsn)
            elif _.scheme in ["mysql", "mariadb"]:
                adapter = adapters.MySQLAdapter(dsn)
