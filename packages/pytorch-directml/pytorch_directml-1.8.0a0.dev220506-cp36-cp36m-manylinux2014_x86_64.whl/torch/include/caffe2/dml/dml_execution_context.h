...
class DmlExecutionContextImpl {
 public:
  ...
  DmlExecutionContextImpl(ID3D12Device* d3d12_device, IDMLDevice* dml_device,
                          ID3D12CommandQueue* queue, ICache* cache);
  ...
  StatusOr<DmlGpuEvent> InitializeOperator(
      ...
      IDmlTensor** out_temporary_resource);

  StatusOr<DmlGpuEvent> ExecuteOperator(
      IDMLCompiledOperator* op,
      const DML_BINDING_DESC& persistent_resource_binding,
      dml::detail::span<const DML_BINDING_DESC> input_bindings,
      dml::detail::span<const DML_BINDING_DESC> output_bindings,
      IDmlTensor** temporary_resource);
  ...
  DmlGpuEvent ResourceBarrier(
      dml::detail::span<const D3D12_RESOURCE_BARRIER> barriers);
...
class DmlExecutionContext {
 public:
  ...
  DmlGpuEvent FillBufferWithPattern(ID3D12Resource* dst, uint64_t dst_offset,
                                    ...
                                    dml::detail::span<const uint8_t> value) {
    std::unique_lock<std::mutex> lock(mutex_);
  ...
  StatusOr<DmlGpuEvent> InitializeOperator(
      ...
      IDmlTensor** out_temporary_resource) {
    std::unique_lock<std::mutex> lock(mutex_);
    return impl_->InitializeOperator(op,
                                     persistent_resource_binding,
                                     input_array_binding,
                                     out_temporary_resource);
  }
  ...
  StatusOr<DmlGpuEvent> ExecuteOperator(
      ...
      IDmlTensor** temporary_resource) {
    std::unique_lock<std::mutex> lock(mutex_);
    return impl_->ExecuteOperator(op,
                                  persistent_resource_binding,
                                  input_bindings,
                                  output_bindings,
                                  temporary_resource);
