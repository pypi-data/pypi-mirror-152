...
class DmlReadbackHeap : public DmlPooledHeap {
 public:
  ...
  StatusOr<DmlGpuEvent> ReadbackFromGpu(
                                        dml::detail::span<uint8_t> dst,
                                        ID3D12Resource* src,
                                        uint64_t src_offset,
 ...
 private:
  std::mutex mutex_;
  DmlExecutionContext* execution_context_;  // weak; owned by DmlBackend
  DmlEventQueue* event_queue_;              // weak; owned by DmlBackend
