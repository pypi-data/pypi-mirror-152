...
#include <caffe2/dml/dml_backend_manager.h>
...
class TORCH_API Context {
 public:
  ...
  void dmlEnsureInit() {
    std::call_once(dml_init, [&] {
      dml_backend_manager_ = dml::DmlBackendManager::Create().release();
    });
  }

  auto getDmlBackend(size_t id) {
    dmlEnsureInit();

    // Sometimes we recieve a dml tensor with no device specified by the engine, specified as -1
    // Select the default backend = 0 if that is the case
    if (id == static_cast<size_t>(-1)) {
      id = 0;
    }

    return dml_backend_manager_->FetchBackend(id);
  }

  auto getDmlBackendName(size_t id) {
    dmlEnsureInit();
    ...
    return dml_backend_manager_->FetchBackendName(id);
  }

  auto getDmlBackendCount() {
    dmlEnsureInit();

    return dml_backend_manager_->FetchBackendsCount();
  }
 ...
 private:
  ...
  dml::DmlBackendManager* dml_backend_manager_;
};
