...
def getUSBDeviceSerialNo( context, device ):
    """ THIS FUNCTION CAUSES A CLOSE PORT """
    ...
    if sNo == 'NO_SERIAL':# or sNo == None:
        ...
        logging.debug(f"Device serial number requested, closing port")
        qDevice.ClosePort()
        sNo = dbgStr.replace("\r\n$","")
    return sNo, qDevice
...
def USB(SerID = ''):
    ...
    for device in context.getDeviceList():
        if device.getVendorID() == QUARCH_VENDOR_ID:
            # MATT Modified this to preserve the TQuarchUSB - stopping multiple open / close attempts on same device
            sNo, QquarchDevice = getUSBDeviceSerialNo( context,  device,  )
            if sNo is None:
                continue
...
class TQuarchUSB_IF:
    ...
    def __del__( self ):
        logging.debug(f"Device reference deleted, closing port")
    ...
    def OpenPort(self):
        ...
        logging.debug(f"Device openPort @ {time.time()}")
        ...
        if self.deviceHandle is not None:
            ...
            if self.QCmdEPSize > 64:
                ...
                try:
                    ...
                    logging.debug(f"Lock command sent, byteCount : {self.lastWriteByteCount}")
                except Exception as e:
                ...
                try:
                    retLineStr = self.deviceHandle.bulkRead( self.QCmdEP, self.QCmdEPSize, self.timeout )
                    logging.debug(f"Lock command response : {str(retLineStr)}")
                except Exception as e:
    ...
    def ClosePort(self):
        ...
        logging.debug(f"Device closePort @ {time.time()}")
        ...
        if self.deviceHandle is not None:
            if not self.QCmdEPSize > 64:  # If this is and Pic32
                try:
                    ...
                    logging.debug(f"Unlock command sent, byteCount : {self.lastWriteByteCount}")
                except Exception as e:
                ...
                try:
                    retLineStr = self.deviceHandle.bulkRead(self.QCmdEP, self.QCmdEPSize, self.timeout)
                    logging.debug(f"Unlock command response : {str(retLineStr)}")
...
class USBConn:
    ...
    def close(self):
        logging.debug(f"Close device function called, closing port")
