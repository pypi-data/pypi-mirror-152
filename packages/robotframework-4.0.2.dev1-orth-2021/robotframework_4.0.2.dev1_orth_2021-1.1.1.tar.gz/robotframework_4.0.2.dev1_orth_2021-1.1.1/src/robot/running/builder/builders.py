...
class TestSuiteBuilder(object):
    ...
    def build(self, *paths,out_dir=None,file=None):
        """
        ...
        suite = parser.parse(structure,out_dir,file)
...
class SuiteStructureParser(SuiteStructureVisitor):
    ...
    def parse(self, structure,out_dir=None,file=None):
        structure.visit(self,out_dir,file)
        self.suite.rpa = self.rpa
        return self.suite

    def visit_file(self, structure,out_dir=None,file=None):
        LOGGER.info("Parsing file '%s'." % structure.source)
        suite, _ = self._build_suite(structure,out_dir,file)
        if self._stack:
            self._stack[-1][0].suites.append(suite)
        else:
            self.suite = suite

    def start_directory(self, structure,out_dir=None):
        if structure.source:
            LOGGER.info("Parsing directory '%s'." % structure.source)
        suite, defaults = self._build_suite(structure,out_dir)
    ...
    def _build_suite(self, structure,out_dir=None,file=None):
        ...
        try:
            if structure.is_directory:
                suite = parser.parse_init_file(structure.init_file or source, defaults,out_dir)
            else:
                suite = parser.parse_suite_file(source, defaults,out_dir,file)
                if not suite.tests:
