...
class RobotParser(BaseParser):
    ...
    def parse_init_file(self, source, defaults=None,out_dir=None):
        directory = os.path.dirname(source)
        suite = TestSuite(name=format_name(directory), source=directory)
        return self._build(suite, source, defaults,out_dir, get_model=get_init_model)

    def parse_suite_file(self, source, defaults=None,out_dir=None,file=None):
        suite = TestSuite(name=format_name(source), source=source)
        return self._build(suite, source, defaults,out_dir,file)
    ...
    def _build(self, suite, source, defaults,out_dir=None,file=None, model=None, get_model=get_model):
        if defaults is None:
