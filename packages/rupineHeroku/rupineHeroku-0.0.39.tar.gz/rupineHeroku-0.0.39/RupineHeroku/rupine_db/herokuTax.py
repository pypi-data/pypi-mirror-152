...
def getTaxTransactionRewards(connection, schema, addresses:list=None, chain=None, chain_id=None, token:str=None, timestamp:int=0, posNegAll:str='all'):
    ...
    query = sql.SQL("SELECT basis.* FROM ( \
                        SELECT t.id,t.chain_id,t.chain,t.public_address,t.timestamp,t.block_number,t.transaction_hash,t.category,t.token,t.amount,t.usd_value,t.eur_value, \
                        ...
                        WHERE 1=1 AND t.timestamp >= %%s %s \
                            UNION ALL	\
                        SELECT t.id,t.chain_id,t.chain,t.public_address,t.timestamp,NULL AS block_number,NULL AS transaction_hash,t.category,t.token,t.amount,t.usd_value,t.eur_value, \
                        CASE WHEN c.sort_no IS NULL THEN '999' ELSE c.sort_no END AS sort_no \
...
def getLatestWarehouseDate(connection, schema, token:str, account:str, chain=None, chain_id=None):
    ...
    query = sql.SQL("SELECT MAX(day) as max_day from {0}.tax_warehouse WHERE 1=1 AND account = %%s AND token = %%s %s;" % (conditions)).format(sql.Identifier(schema))
...
#     )
    ...
    # res = getLatestWarehouseDate(connection,'stage','BLUE1','MYACC','ETH',None)
    # print(res.timestamp())