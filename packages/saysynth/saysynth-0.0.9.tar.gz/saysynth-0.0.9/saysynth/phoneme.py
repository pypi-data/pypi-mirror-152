...
def _get_phoneme_duration(
    phoneme_duration, phoneme_bpm=None, phoneme_count=1, phoneme_time_sig="4/4"
):
    """
    calculate phoneme duration or pass it through
    """
    if phoneme_bpm:
        return bpm_to_time(phoneme_bpm, phoneme_count, phoneme_time_sig)
...
def text_from_note(
    ...
    **kwargs,
    ...
    if duration <= phoneme_duration:
        return format_text(
            phoneme, duration, frequency, emphasis, volume_tag, is_silence
        )
    ...
    for dur_step in frange(
        ...
        text = format_text(
            phoneme, phoneme_duration, frequency, emphasis, volume_tag, is_silence
        )
        texts.append(text)

    # add final step and return
    text = format_text(
        phoneme, (duration - dur_step), frequency, emphasis, volume_tag, is_silence
    )
    texts.append(text)
    return f" ".join(texts) + " "
