...
class DingTalkPlugin(CorePluginMixin, notify.NotificationPlugin):
    ...
    def notify(self, notification, raise_exception=False):
        ...
        time = event.datetime + event.datetime.timedelta(hours=8)
        datetime = time.strftime("%m-%d %T")
        self._post(group, user_id, release, project,datetime)

    def _post(self, group, userId, release, project,datetime):
        ...
        payload = f"{payload} #### 触发时间：{datetime} \n\n"
        ...
        data = {
            "msgtype": "markdown",
            "markdown": {
                "title": f"{project.name}发生告警：{group.message};问题地址：{issue_link}",
                "text": payload,
            },
            "at": {
