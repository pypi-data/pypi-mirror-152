...
class NestedFlowExecution(unittest.TestCase):
    @classmethod
    ...
    def test_end_to_end(self):
        client = NestedFlowExecution._client
        seq1 = NestedFlowExecution._sequence1
        seq2 = NestedFlowExecution._sequence2
        ...
        result_flow = client.get_sequence_data(client.run(dsl_op.my_flow2(seq1, seq2, -1)))
        result_no_flow = client.get_sequence_data(client.run(no_flow(seq1, seq2, -1)))
        result_expected = create_small_df([3.1, 10.4, 23.6, 464.0, 49.5]).iloc[:, 0]
        pd.testing.assert_series_equal(result_flow, result_expected, check_names=False)
        pd.testing.assert_series_equal(result_flow, result_no_flow, check_names=False)
