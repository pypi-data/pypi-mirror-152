...
class CommonSel(SelMixin):
    ...
    def __init__(self, remote=None, phone=False, no_img=False, proxy=None, channel_proxy=False,
        ...
        if platform.system() == 'Darwin':
            ...
            self.driver = webdriver.Chrome(
                executable_path=f"{os.path.join(base_path, 'resources/chromedriver')}",
                chrome_options=chrome_options
            )
        elif platform.system() == 'Linux':
            # centos
            self.driver = webdriver.Chrome(
                executable_path=f"{os.path.join(base_path, 'resources/chromedriver_centos')}",
                chrome_options=chrome_options
            )
        else:
            # windows
            self.driver = webdriver.Chrome(
                executable_path=f"{os.path.join(base_path, 'resources/chromedriver.exe')}",
                chrome_options=chrome_options
            )
    ...
    def debug(self, timeout=10000000):
        """
        调试
        Args:
            timeout:
