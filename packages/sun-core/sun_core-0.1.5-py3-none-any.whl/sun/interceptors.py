...
from aio.interceptors import ServerInterceptor as AioServerInterceptor
...
class AioProcessInterceptor(AioServerInterceptor, ABC):
    def setup(self):
        pass
    ...
    def intercept(
        ...
        self.setup()
        try:
            _settings.ENABLED_RPC_LOGGING and self.log(request, method_name,
                                                       'Request')
            result = method(request, context)
            _settings.ENABLED_RPC_LOGGING and self.log(request, method_name,
                                                       'Response')
