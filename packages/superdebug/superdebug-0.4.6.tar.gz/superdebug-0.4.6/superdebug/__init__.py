...
CALL_CHAIN_LEN = 0 # 标准为0，即只显示调用debug()的位置，设置成>0则可显示更长的调用链
        ...
        if var is None:
            ...
            if layer < PEEK_LAYER and len(var) > 0:
                ...
                for no, item in enumerate(var):
                    info(item, "item " + str(no) + ": ", detail, layer + 1)
                if type(var) == list:
                    logging(space * layer, end="and extra items")
            else:
                logging(" val:", var if detail else "*")
        elif type(var) == dict:
            ...
            logging(space * layer, f"\033[0m\033[1;36m{name}\033[0m\033[1;33m", "dict {" + min(len(var), 3)*'.' + "} " + f"with {len(var)} keys", dict_keys, end=dict_end)
            if layer < PEEK_LAYER and len(var) > 0:
                logging("")
            ...
            if len(dict_keys) >= 100:
                dict_keys = dict_keys[:100]
                dict_end = "... and extra items]"
            logging(space * layer, f"\033[0m\033[1;36m{name}\033[0m\033[1;33m", "OrderedDict {" + min(len(var), 3)*'.' + "} " + f"with {len(var)} keys", dict_keys, end=dict_end)
            if layer < PEEK_LAYER and len(var) > 0:
                logging("")
                for key in dict_keys:
                    info(var[key], key, detail, layer + 1)
            ...
            logging(space * layer, f"\033[0m\033[1;36m{name}\033[0m\033[1;33m", "defaultdict {" + min(len(var), 3)*'.' + "} with default", default_val, f"{len(var)} keys", dict_keys, end=dict_end)
            if layer < PEEK_LAYER and len(var) > 0:
                logging("")
                for key in dict_keys:
                    info(var[key], key, detail, layer + 1)
