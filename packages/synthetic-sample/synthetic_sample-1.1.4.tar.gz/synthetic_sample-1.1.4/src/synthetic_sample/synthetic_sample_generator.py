...
def synthetic_sample(input_dict: dict, create_records_dataset: bool = False) -> dict:
    ...
    if create_records_dataset:
        ...
        if type(period_definition) == tuple:
            # If specific dates are requested, drop sample outside of bounds
            mask = sample_records_df["date"] < request_end_date
            mask = mask & (sample_records_df["date"] >= request_start_date)
            sample_records_df = sample_records_df[mask]

            # Reallocate data from ISO years to calendar years for start and end of dataset
            input_start_date = input_dict.get("start_date")
            input_end_date = input_dict.get("end_date")
            if (input_start_date.month == 1) & (input_start_date.day == 1):
                mask = sample_records_df["date"] < input_start_date
                sample_records_df.loc[mask, "date"] = input_start_date
            if (input_end_date.month == 12) & (input_end_date.day == 31):
