...
May 25, 2022 Version 0.1.7: Removed leading space in the returned result when using Julia.

May 15, 2022 Version 0.1.6: Removed leading space in the returned result.

May 3, 2022 Version 0.1.5: when used to communicate with a python engine we initialize it with python3 -iqu to supress the banner.
Also, to execute a file when using a python engine we use exec(open(somefile).read()). In the previous versions it was implemented via a lambda expression, but that meant that variables/functions defined in the input file are not available after the execution.
