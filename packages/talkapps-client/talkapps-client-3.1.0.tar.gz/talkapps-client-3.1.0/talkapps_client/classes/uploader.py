...
from talkapps_client.classes.talkcrypto import encrypt, derive_key
from talkapps_client.urls import SERVICE_STICKER_FORM_URL, CDN_BASEURL
from talkapps_client.errors import HTTPException, Unauthorized, RateLimited


async def upload_pack(http, pack, talk_user, talk_password):
    """
    Upload a pack, and return (pack_id, pack_key)
    """

    if not talk_user or not talk_password:
        raise ValueError("talk_user or talk_password not set")
