...
"""
    Python client for the Talk stickers API.
    If you want to understand this code, you must be familiar with Talk
    stickers internals. tl;dr:
     - Everything in packs is encrypted on Talk's side;
     ...
     - Order of operations is:
      ...
      This is what happens when you open a sticker pack in your Talk client,
      or what we reproduce on talkapps.github.io/sticker-store .


    Please do not flood the Talk API (so don't put this code in a `while True` loop).
...
from talkapps_client.classes import downloader, uploader
from talkapps_client.models import LocalStickerPack
from talkapps_client.utils.ca import CACERT_PATH


class StickersClient:
    def __init__(self, talk_user="", talk_pass=""):
        self.http: httpx.AsyncClient
        self.talk_user = talk_user
        self.talk_pass = talk_pass
    ...
    async def upload_pack(self, pack: LocalStickerPack):
        ...
        """
        return await uploader.upload_pack(self.http, pack, self.talk_user, self.talk_pass)
