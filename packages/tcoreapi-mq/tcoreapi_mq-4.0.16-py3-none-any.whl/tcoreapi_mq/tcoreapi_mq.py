...
class QuoteClass():
                ...
                if self.getstatus(symbol, bartype, startTime, endTime,dbtype)=='Ready':
                    ...
                    while True:
                        ...
                        if his and len(his["HisData"])>0:
                            ...
                            #self.status.remove({'DataType': m_type, 'Symbol': symbol, 'Status': 'Ready'})
    ...
    def gethistorymulti(self,symbol, type, startTime, endTime,dbtype=""):
        ...
        return result
    def __gethis(self,symbol, type, startTime, endTime='time.strftime("%Y%m%d",time.localtime())+"07"',dbtype=""):
        self.__subHistory(symbol, type, startTime, endTime,dbtype)
        while True:
            if self.getstatus(symbol, type, startTime, endTime,dbtype)=='Ready':
                temp=[]
                if dbtype=="ODBC":
                    if type=="DOGSK" or type=="DOGSS":
                ...
                while True:
                    his=self.__get_history(symbol, type, startTime, endTime,last)
                    if his and len(his["HisData"])>0:
