    ...
    {%- if include_feature_view_timestamp_columns -%}
        {%- for item in feature_set_items -%}
            {%- if item.aggregation -%}
                ...
                DATEADD('SECOND', -MOD(DATE_PART(EPOCH_SECOND, _TT_SPINE_TABLE.{{ spine_timestamp_key }}), {{ item.aggregation.slide_interval.ToSeconds() }}), _TT_SPINE_TABLE.{{ spine_timestamp_key }}) AS {{ final_select_name(item.name + separator + item.timestamp_key, false, true) }}
            {%- else %}
