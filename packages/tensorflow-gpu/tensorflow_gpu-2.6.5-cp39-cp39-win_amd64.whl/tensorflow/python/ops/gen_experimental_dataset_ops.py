...
def data_service_dataset(dataset_id, processing_mode, address, protocol, job_name, max_outstanding_requests, iteration_counter, output_types, output_shapes, task_refresh_interval_hint_ms=-1, data_transfer_protocol="", target_workers="AUTO", name=None):
  ...
  if tld.is_eager:
    ...
    try:
      return data_service_dataset_eager_fallback(
          ...
          target_workers=target_workers, name=name, ctx=_ctx)
    except _core._SymbolicException:
      pass  # Add nodes to the TensorFlow graph.
  # Add nodes to the TensorFlow graph.
  if not isinstance(output_types, (list, tuple)):
  ...
  _, _, _op, _outputs = _op_def_library._apply_op_helper(
        "DataServiceDataset", dataset_id=dataset_id,
                              ...
                              task_refresh_interval_hint_ms=task_refresh_interval_hint_ms,
                              data_transfer_protocol=data_transfer_protocol,
                              target_workers=target_workers, name=name)
  _result = _outputs[:]
  if _execute.must_record_gradient():
...
def data_service_dataset_v2(dataset_id, processing_mode, address, protocol, job_name, consumer_index, num_consumers, max_outstanding_requests, iteration_counter, output_types, output_shapes, task_refresh_interval_hint_ms=-1, data_transfer_protocol="", target_workers="AUTO", name=None):
  r"""Creates a dataset that reads data from the tf.data service.
