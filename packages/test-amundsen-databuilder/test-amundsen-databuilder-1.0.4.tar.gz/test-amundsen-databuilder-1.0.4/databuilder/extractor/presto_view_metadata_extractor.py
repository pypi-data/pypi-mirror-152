...
class PrestoViewMetadataExtractor(Extractor):
    ...
    def init(self, conf: ConfigTree) -> None:
        ...
        LOGGER.info(conf)
        LOGGER.info(type(conf))
        for item in conf.items():
            LOGGER.info(item)
        LOGGER.info(conf.get("extractor").get("sqlalchemy").get("conn_string"))    
        LOGGER.info(conf.get("extractor.sqlalchemy.conn_string"))
        conn_string = conf.get_string(SQLAlchemyExtractor.CONN_STRING)
        LOGGER.info(conn_string)
        url = make_url(conn_string)
        LOGGER.info(url.drivername.lower())
