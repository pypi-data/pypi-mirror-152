...
class HiveTableLastUpdatedExtractor(Extractor):
    ...
    def _get_non_partitioned_table_sql_alchemy_extractor(self) -> Extractor:
        ...
        sql_stmt = self._choose_default_non_partitioned_sql_stm().format(
    ...
    def _choose_default_non_partitioned_sql_stm(self) -> str:
        url = make_url(self._conf.get_string("extractor.sqlalchemy.conn_string"))
        if url.drivername.lower() in ['postgresql', 'postgres']:
            return self.DEFAULT_POSTGRES_NON_PARTITIONED_TABLE_SQL_STATEMENT
        else:
            return self.DEFAULT_NON_PARTITIONED_TABLE_SQL_STATEMENT
