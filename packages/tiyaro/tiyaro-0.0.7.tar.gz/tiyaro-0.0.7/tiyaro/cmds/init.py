...
def get_model_class(is_verbose):
    while True:
        ...
        option = click.prompt(
            f'Please enter the class of your model {framework_opt} \t\t\t', type=str)
        option = option.casefold()

        if option in ['1', 'image-classification']:
            option = 'image-classification'
            break
        elif option in ['2', 'custom']:
            option = 'custom'
            break
        else:
...
def filter_class_specific_templates(clazz, is_verbose):
    ...
    for file in class_template_files:
        if (os.path.isfile(file)):
            os.remove(file)
