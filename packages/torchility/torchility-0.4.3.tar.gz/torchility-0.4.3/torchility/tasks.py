...
class GeneralTaskModule(LightningModule):
    ...
    def training_step(self, batch, batch_nb):               # 训练步
        ...
        self.log('train_loss', loss, prog_bar=True, on_step=True, on_epoch=True, batch_size=preds.shape[0])
    ...
    def validation_step(self, batch, batch_nb):             # 验证步
        loss, preds, targets = self.do_forward(batch)
        self.log('val_loss', loss, prog_bar=True, on_step=True, on_epoch=True, batch_size=preds.shape[0])
    ...
    def test_step(self, batch, batch_nb):                   # 测试步
        loss, preds, targets = self.do_forward(batch)
        self.log('test_loss', loss, prog_bar=True, on_step=True, on_epoch=True, batch_size=preds.shape[0])
