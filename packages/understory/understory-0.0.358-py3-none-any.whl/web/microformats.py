"""
[Microformats][0] parser and utilities.
...
def parse(doc=None, url=None):
    ...
    if doc is None:
        ...
        except agent.SSLError:
            response = agent.get(url, verify=False)
        doc = response.text
...
def representative_card(parsed, source_url):
    ...
    if match := _check_uid_and_url_match_source_url(source_url, cards):
        return match
    if match := _check_url_matches_rel_me(parsed, cards):
        return match
...
def _check_url_matches_rel_me(parsed, cards):
    for card in cards:
        rel_mes = set()
