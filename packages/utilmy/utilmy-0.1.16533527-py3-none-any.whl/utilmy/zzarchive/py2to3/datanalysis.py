...
import sys, os, numpy as np, pandas as pd, copy, scipy as sci, matplotlib.pyplot as plt, math as mth
import requests, re
...
def xl_get_rowcol(ws, i0, j0, imax, jmax) :
   rmat= []
   for j in xrange(j0, j0+jmax) :
     if isnull(xl_val(ws, j, i0)) and isnull(xl_val(ws, j+1, i0))  : #Stop conditions : column j+1 is empty
       return rmat
     rmatj=  [xl_val(ws, j, i)  for i in xrange(i0, i0+imax)  ]   # add all the rows
     rmat.append(rmatj)
   return rmat
...
def xl_getschema(dirxl="", filepattern='*.xlsx', dirlevel=1, outfile='.xlsx') :
 ...
 def xl_find_start_block(ws, colmin, rowmin, colmax, rowmax):
       ...
       for j in xrange(colmin, colmax) :
         for i in xrange(rowmin, rowmax) :
 ...
 def _xl_getschema(file1="*.xlsx",  output_istack=1) :
  ...
  for wsname in ws_names :
    ...
    if ws.max_column > 2   and ws.max_row > 3 :   #Minimum table size
     ...
     nlen=  xmat.shape[0]
     xfile= np.array( [[ filepath1, filetype1, filen1, unicode(wsname) ]]  * nlen)
...
3. Encode late :f = open('/tmp/ivan_out.txt','w')
 ...
 for ii, tablei in enumerate(csvlist) :
   ...
   df_type2=       [ str(type(df.iloc[0,i])) for i in xrange(0, ncol) ]
   ...
   for i, x in enumerate(df_type1) :
      ...
      if str(x).find('int') > -1 :
         ...
         elif df_max[i] < 4294967296L: df_type_guess[i] = 'int32'
...
def csv_row_reduce_line(fromfile, tofile, condfilter, catval_tokeep, maxline=-1):
...
def csv_row_mapreduce(dircsv="", outfile="", type_mapreduce='sum', chunk= 5000000) :
...
def db_meta_find(ALLDB, query='', filter_db=[],   filter_table=[], filter_column=[] ) :
  ...
  '''
  rs=[]
  for iname, dbi in ALLDB.items() :      #Search in All DB
    if iname in filter_db or len(filter_db)==0 :
      for jname, tablej in dbi.items() :    # inside sub-meta table of DB
...
def col_study_distribution_show(df, col_include=None, col_exclude=None, pars={'binsize':20}):
 ...
 for feature in features:
    ...
    plt.plot(range(5,100,5), np.percentile(values, range(5,100,5)),'.b')
    plt.show()
...
def col_pair_plot(dfX, Xcolname_selectlist=None, dfY=None, Ycolname=None) :
...
def plot_cluster_2D(X_2dim, target_class, target_names):
   ...
   target_ids=xrange(0, len(target_names))
   pl.figure()
...
def plot_cluster_hiearchy(Xmat_dist, p=30, truncate_mode=None, color_threshold=None,  get_leaves=True, orientation='top', labels=None,  count_sort=False, distance_sort=False, show_leaf_counts=True, no_plot=False, no_labels=False, leaf_font_size=None, leaf_rotation=None, leaf_label_func=None, show_contracted=False, link_color_func=None, ax=None,
   ...
   ddata= dendrogram(Xmat_dist, p=30, truncate_mode= truncate_mode, color_threshold=color_threshold,
                     ...
                     no_plot=no_plot, no_labels=False, leaf_font_size=None,
...
def sk_params_search_best(Xmat, Ytarget, model1, param_grid={'alpha':  np.linspace(0,1, 5) }, method='gridsearch',
  ...
  if method=='gridsearch' :
    ...
    grid = GridSearchCV(model1,param_grid, cv=cv, scoring=myscore) # 20-fold cross-validation
...
kde.sample(5)
   ...
   if method=='kmeans' :
    ...
    kmeans = k_means(*args, **kwds)    #  KMeans(n_clusters= nbcluster)
    kmeans.fit(Xmat)
...
class sk_stateRule() :
  ...
  def __init__(self, state, trigger, colname=[]):
    ...
    else :                    self.colname= [ 'a'+str(i) for i in xrange(0, self.nstate ) ]
