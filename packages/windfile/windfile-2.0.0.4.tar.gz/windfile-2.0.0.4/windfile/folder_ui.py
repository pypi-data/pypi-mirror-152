...
class FolderWidgets(Frame):
    ...
    def on_tree_click(self, event):
        ...
        self.statusLabel.config(text="%s"%(self.ui_id_node[selected[0]].fullpath if selected else ""))            
    ...
    def selecteFilesInSameFolder(self):
        ...
        # count = self.selected_count()
        self.statusLabel.config(text="{} ".format(parent.fullpath)) 
        pass
        ...
        if confirm or askyesnocancel("真的要删除这些文件吗？",message[0:1000]):
            for f in files:
                ...
                # if self.ui_id_node[f].ui_id == f:
                    self.ui_id_node[f].remove()
                    self.dirTreeview.delete(f)
                self.ui_id_node.pop(f)
        count = self.selected_count()
        self.statusLabel.config(text="{} folder {} files total size {} bytes deleted!".format(count[0],count[1],count[2])) 
        return 
    ...
    def copyFiles(self, files:set=()):
        ...
        count = self.selected_count()
        self.statusLabel.config(text="{} folder {} files total size {} bytes copyed!".format(count[0],count[1],count[2])) 
        pass
    ...
    def moveFiles(self,files:set=()):
        ...
        count = self.selected_count()
        self.statusLabel.config(text="{} folder {} files total size {} bytes moved!".format(count[0],count[1],count[2])) 
