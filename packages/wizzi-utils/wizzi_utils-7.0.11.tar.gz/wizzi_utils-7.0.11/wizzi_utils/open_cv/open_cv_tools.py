...
class CameraWu:
    def __init__(self, port: (int, str), type_cam: str = 'cv2'):
        """
    ...
    def set_resolution(self, w: int = 640, h: int = 480, ack: bool = False) -> None:
        """
        :param w: width
        :param h: height
        :param ack:
        ...
        :return:
        """
        old_w, old_h = self.get_resolution(ack=False)
        ...
        if self.type_cam == 'cv2':
            ...
            pass
        if ack:
            print('\tport {}: {}x{} -> {}x{}'.format(self.port, old_w, old_h, w, h))
        return
