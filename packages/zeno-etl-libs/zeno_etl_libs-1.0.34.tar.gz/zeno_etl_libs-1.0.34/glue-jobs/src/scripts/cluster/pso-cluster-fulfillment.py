...
parser.add_argument('-wt', '--write_to_mysql', default="0", type=str, required=False)
...
if start1=="NULL" and start2=="NULL":
    ...
    if datetime.now(tz=gettz('Asia/Kolkata')).strftime('%H:%M:%S') < '09:30:00':
        start1 = (datetime.now(tz=gettz('Asia/Kolkata')) -
                  timedelta(days=1)).strftime('%Y-%m-%d 19:30:00')
        ...
        if env=='dev':
            ...
            def ceil_dt(dt, delta):
                return dt + (datetime(1, 1, 1, 0, 0, 0 ,tzinfo=gettz('Asia/Calcutta'))  - dt) % delta - timedelta(minutes=6,seconds=40)
