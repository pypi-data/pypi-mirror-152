...
# Absolute changes
df_abs = df_filtered.copy()
params_list = list(df_abs['params'])
param_value_list = list(df_abs['parameter value'])
column_list = list(df_abs.columns)[2:]
# Iterating through all the entries in the final dataframe
for c in column_list:
    for (p,pv) in zip(params_list,  param_value_list):
        df_abs[c] = np.where((df_abs['params'] == p) & (df_abs['parameter value'] == pv), absolute(p,pv,c), df_abs[c])
...
ws.write(20, 0, "Absolute Changes", cell_format_bg_yl)
limit3 = df_abs_rev.shape[1] + 1
row_index = 21
